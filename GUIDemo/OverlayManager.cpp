...
//-----------------------------------------------------------------------------
tOverlayManager::tOverlayManager() { Clear(); }

void tOverlayManager::Clear() { memset(&m_Overlays, 0, sizeof(m_Overlays)); }

void tOverlayManager::SetGuardZone(int index, bool enabled,
                                   uint32_t startRange_m, uint32_t endRange_m,
...
void tOverlayManager::SetBlankingSector(int index, bool enabled,
                                        uint16_t bearingStart,
                                        uint16_t bearingEnd) {
  if (index < Navico::Protocol::NRP::cMaxBlankSectors) {
    tOverlay& overlay =
        m_Overlays[Navico::Protocol::NRP::cMaxGuardZones + index];
    overlay.enabled = enabled;
