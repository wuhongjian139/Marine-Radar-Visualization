...
void runge_kutta_4(
    ...
    const differ_state<state_type> &inputs, const double time_start,
    const double time_end, const std::size_t steps, const bool flag_collect,
    const differ_extra<extra_type>
        &argv, /* additional arguments that are used in the equation */
  ...
  for (std::size_t i = 0; i < steps; i++) {
    ...
    func(current_result.time + step / 2.0, current_result.state + k1 / 2.0,
         argv, fp2);
    k2 = fp2 * step;

    func(current_result.time + step / 2.0, current_result.state + k2 / 2.0,
         argv, fp3);
    k3 = fp3 * step;
