...
void runge_kutta_fehlberg_45(const equation<state_type, extra_type> &func,
                             const differ_state<state_type> &inputs,
                             const double time_start, const double time_end,
                             const double tolerance, const bool flag_collect,
  ...
  while (current_result.time < time_end) {
    ...
    k4 = h * fp4;
    y5 = current_value + factor::B5 * k1 + factor::C5 * k2 + factor::D5 * k3 +
         factor::E5 * k4;

    func(current_time + factor::A5 * h, y5, argv, fp5);
    k5 = h * fp5;
    y6 = current_value + factor::B6 * k1 + factor::C6 * k2 + factor::D6 * k3 +
         factor::E6 * k4 + factor::F6 * k5;

    func(current_time + factor::A6 * h, y6, argv, fp6);
    k6 = h * fp6;

    /* Calculate error (difference between Runge-Kutta 4 and Runge-Kutta 5) and
     * new value. */
    differ_state<state_type> errors = factor::R1 * k1 + factor::R3 * k3 +
                                      factor::R4 * k4 + factor::R5 * k5 +
                                      factor::R6 * k6;

    double err = 0.0;
    for (typename differ_state<state_type>::const_iterator iter =
             errors.cbegin();
         iter != errors.cend(); ++iter) {
    ...
    if ((err < tolerance) || (h < 2.0 * hmin)) {
      /* Calculate new value. */
      current_result.state = current_value + factor::N1 * k1 + factor::N3 * k3 +
                             factor::N4 * k4 + factor::N5 * k5;
