 ...
 * @param[in] p_arguments: additional arguments, for example, degree in case of
 * minkowski distance.
 * @param[in] p_solver: pointer to user-defined function that should be used for
 * calculation, used only in case of 'USER_DEFINED' metric type.
 *
 * @return  Returns pointer to metric object, returned object should be
 * destroyed by 'metric_destroy'.
 *
 */
extern "C" DECLARATION void *metric_create(
    const std::size_t p_type, const pyclustering_package *const p_arguments,
