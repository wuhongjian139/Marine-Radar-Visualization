 ...
 * @details  Caller should destroy created instance by 'som_destroy' when it is
 * not required.
 *
 * @param[in] num_rows: amount of neurons in each row of the map.
 * @param[in] num_cols: amount of neurons in each column of the map.
 * @param[in] type_conn: type of connections between neurons (grid-four,
 * grid-eight, honeycomb, defined by neighbor function, etc.).
 ...
 *
 */
extern "C" DECLARATION void* som_create(const size_t num_rows,
                                        const size_t num_cols,
                                        const size_t type_conn,
...
/**
 *
 * @brief    Load network parameters such weights, amount of won objects by each
 * neuron (optional), captuted objects by each neuron (optional).
 *
 ...
 * @param[in] p_awards: amount of captured objects by each neuron (optional
 * parameter and can be 'nullptr').
 * @param[in] p_captured_objects: captured objects by each neuron (optional
 * parameter and can be 'nullptr').
 *
 * @see  som_destroy
 *
 */
extern "C" DECLARATION void som_load(
    const void* p_pointer, const pyclustering_package* p_weights,
    const pyclustering_package* p_awards,
    const pyclustering_package* p_captured_objects);
 ...
 *
 */
extern "C" DECLARATION size_t
som_train(const void* pointer, const pyclustering_package* const sample,
          const size_t epochs, const bool autostop);
 ...
 */
extern "C" DECLARATION size_t
som_simulate(const void* pointer, const pyclustering_package* const p_pattern);

/**
 *
 * @brief  Returns number of neuron winners at the last step of learning
 * process.
 *
 * @param[in] pointer: pointer to instance of self-organized feature map.
 *
 ...
 *
 */
extern "C" DECLARATION pyclustering_package* som_get_weights(
    const void* pointer);
 ...
 *
 */
extern "C" DECLARATION pyclustering_package* som_get_capture_objects(
    const void* pointer);
 ...
 *
 */
extern "C" DECLARATION pyclustering_package* som_get_awards(
    const void* pointer);
 ...
 *
 */
extern "C" DECLARATION pyclustering_package* som_get_neighbors(
    const void* pointer);
