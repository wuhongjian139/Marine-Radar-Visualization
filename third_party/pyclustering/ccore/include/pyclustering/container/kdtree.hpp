...
/**
 *
 * @brief   KD Tree - structure for storing data where fast distance searching
 * is required.
 *
 */
class kdtree {
 ...
 private:
   ...
   * @param[in] p_discriminator: discriminator that is used for comparison of
   * nodes.
   *
   * @return  Return the smallest node in specified subtree in line with
   * discriminator.
   *
   */
  static kdnode::ptr find_minimal_node(const kdnode::ptr &p_cur_node,
                                       const std::size_t p_discriminator);

  /**
   *
   * @brief   Find node using specified rule, it returns the first node that
   * satisfy search rule.
   *
   ...
   *
   */
  static kdnode::ptr find_node_by_rule(const std::vector<double> &p_point,
                                       const kdnode::ptr &p_cur_node,
                                       const search_node_rule &p_rule);
 ...
 public:
   ...
   * @param[in] p_payload: payloads of node (can be nullptr if it's not
   * required).
   *
   * @return  Pointer to added node in the tree.
   *
   */
  kdnode::ptr insert(const std::vector<double> &p_point,
   ...
   * @details This remove is useful when points with the same coordinates are
   * located in the tree and but only one of them should be removed with
   * specific payload where node ID or other unique information is stored.
   *
   * @param[in] p_point: coordinates that describe node in tree.
   ...
   *
   */
  kdnode::ptr find_node(const std::vector<double> &p_point,
                        const void *p_payload) const;

  /**
   *
   * @brief   Find node in tree using coordinates in subtree, the first founded
   * node with specified coordinates is returned.
   *
   * @param[in] p_point: coordinates of searched node.
   ...
   *
   */
  static kdnode::ptr find_node(const std::vector<double> &p_point,
                               const kdnode::ptr &p_cur_node);

  /**
   *
   * @brief   Traverse tree from specified node and returns number of nodes in
   * subtree.
   *
...
/**
 *
 * @brief   Searcher in KD Tree provides services related to searching in KD
 * Tree.
 *
 */
class kdtree_searcher {
 ...
 public:
  /**
   *
   * @brief   Default constructor. Search will not be performed until it's
   * initialized.
   *
   */
   ...
   *
   */
  kdtree_searcher(const std::vector<double> &point, const kdnode::ptr &node,
 ...
 public:
  /**
   *
   * @brief   Search nodes that are located in specified distance from specified
   * point.
   *
   * @param[out] p_distances: distances from the point to nodes in the location
   * (that are radius-reachable).
   ...
   *
   */
  void find_nearest_nodes(std::vector<double> &p_distances,
                          std::vector<kdnode::ptr> &p_nearest_nodes) const;

  /**
   *
   * @brief   Search the nearest node in specified location for specified point
   * in the request.
   *
   * @return  Return pointer to the nearest node in kd tree that satisfy the
   * request.
   *
   */
  ...
  /**
   *
   * @brief   Search the nearest nodes and store information about found node
   * using user-defined way.
   *
 ...
 private:
   ...
   *
   */
  void initialize(const std::vector<double> &point, const kdnode::ptr &node,
                  const double radius_search);
   ...
   *
   * @param[in] node: initial node in tree from which searching should
   * performed.
   *
   */
   ...
   *
   * @param[in] node: node that should be added to best collection if it is
   * reachable.
   *
   */
