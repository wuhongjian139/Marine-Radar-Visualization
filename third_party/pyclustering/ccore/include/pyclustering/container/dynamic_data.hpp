...
/**
 *
 * @brief   Collection that stores dynamic of oscillatory network - state of
 * each oscillator on each iteration.
 *
 */
...
class dynamic_data : public std::vector<DynamicType> {
 ...
 public:
  ...
  explicit dynamic_data(const std::size_t p_size)
      : std::vector<DynamicType>(p_size), m_oscillators(0) {}

  dynamic_data(const std::size_t p_size, const DynamicType& p_value)
      : std::vector<DynamicType>(p_size, p_value), m_oscillators(0) {}
 ...
 public:
  ...
  std::size_t oscillators() const { return m_oscillators; }
  ...
  void check_set_oscillators(const DynamicType& p_value) {
    if (std::vector<DynamicType>::empty()) {
      ...
      throw std::range_error(
          "Dynamic collection can consist of network states with the same size "
          "only");
    }
  }
