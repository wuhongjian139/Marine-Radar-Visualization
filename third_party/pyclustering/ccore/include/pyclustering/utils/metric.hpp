...
/**
 *
 * @brief   Encapsulates distance metric calculation function between two
 * objects.
 *
 */
template <typename TypeContainer>
using distance_functor =
    std::function<double(const TypeContainer&, const TypeContainer&)>;
 ...
 */
template <typename TypeContainer>
double euclidean_distance_square(const TypeContainer& point1,
                                 const TypeContainer& point2) {
  double distance = 0.0;
 ...
 */
template <typename TypeContainer>
double euclidean_distance(const TypeContainer& point1,
                          const TypeContainer& point2) {
 ...
 */
template <typename TypeContainer>
double manhattan_distance(const TypeContainer& point1,
                          const TypeContainer& point2) {
  double distance = 0.0;
  typename TypeContainer::const_iterator iter_point1 = point1.begin();
 ...
 */
template <typename TypeContainer>
double chebyshev_distance(const TypeContainer& point1,
                          const TypeContainer& point2) {
  double distance = 0.0;
  typename TypeContainer::const_iterator iter_point1 = point1.begin();
 ...
 */
template <typename TypeContainer>
double minkowski_distance(const TypeContainer& p_point1,
                          const TypeContainer& p_point2,
                          const double p_degree) {
  double distance = 0.0;
 ...
 */
template <typename TypeContainer>
double canberra_distance(const TypeContainer& point1,
                         const TypeContainer& point2) {
  double distance = 0.0;
  typename TypeContainer::const_iterator iter_point1 = point1.begin();
 ...
 */
template <typename TypeContainer>
double chi_square_distance(const TypeContainer& point1,
                           const TypeContainer& point2) {
  double distance = 0.0;
  typename TypeContainer::const_iterator iter_point1 = point1.begin();
 ...
 */
template <typename TypeContainer>
double gower_distance(const TypeContainer& p_point1,
                      const TypeContainer& p_point2,
                      const TypeContainer& p_max_range) {
  double distance = 0.0;
  typename TypeContainer::const_iterator iter_point1 = p_point1.begin();
...
/**
 *
 * @brief   Basic distance metric provides interface for calculation distance
 * between objects in line with specific metric.
 *
 */
template <typename TypeContainer>
class distance_metric {
 ...
 public:
  ...
  explicit distance_metric(const distance_functor<TypeContainer>& p_functor)
      : m_functor(p_functor) {}
 ...
 public:
   ...
   *
   */
  double operator()(const TypeContainer& p_point1,
                    const TypeContainer& p_point2) const {
 ...
 public:
  ...
  distance_metric<TypeContainer>& operator=(
      const distance_metric<TypeContainer>& p_other) {
    if (this != &p_other) {
...
class euclidean_distance_metric : public distance_metric<TypeContainer> {
 public:
  euclidean_distance_metric()
      : distance_metric<TypeContainer>(
            std::bind(euclidean_distance<TypeContainer>, std::placeholders::_1,
                      std::placeholders::_2)) {}
};
...
class euclidean_distance_square_metric : public distance_metric<TypeContainer> {
 public:
  euclidean_distance_square_metric()
      : distance_metric<TypeContainer>(
            std::bind(euclidean_distance_square<TypeContainer>,
...
class manhattan_distance_metric : public distance_metric<TypeContainer> {
 public:
  manhattan_distance_metric()
      : distance_metric<TypeContainer>(
            std::bind(manhattan_distance<TypeContainer>, std::placeholders::_1,
                      std::placeholders::_2)) {}
};
...
class chebyshev_distance_metric : public distance_metric<TypeContainer> {
 public:
  chebyshev_distance_metric()
      : distance_metric<TypeContainer>(
            std::bind(chebyshev_distance<TypeContainer>, std::placeholders::_1,
                      std::placeholders::_2)) {}
};
...
class minkowski_distance_metric : public distance_metric<TypeContainer> {
 public:
  ...
  explicit minkowski_distance_metric(const double p_degree)
      : distance_metric<TypeContainer>(
            std::bind(minkowski_distance<TypeContainer>, std::placeholders::_1,
                      std::placeholders::_2, p_degree)) {}
};
...
class canberra_distance_metric : public distance_metric<TypeContainer> {
 public:
  canberra_distance_metric()
      : distance_metric<TypeContainer>(
            std::bind(canberra_distance<TypeContainer>, std::placeholders::_1,
                      std::placeholders::_2)) {}
};
...
class chi_square_distance_metric : public distance_metric<TypeContainer> {
 public:
  chi_square_distance_metric()
      : distance_metric<TypeContainer>(
            std::bind(chi_square_distance<TypeContainer>, std::placeholders::_1,
                      std::placeholders::_2)) {}
};
...
class gower_distance_metric : public distance_metric<TypeContainer> {
 public:
  ...
  explicit gower_distance_metric(const TypeContainer& p_max_range)
      : distance_metric<TypeContainer>(
            std::bind(gower_distance<TypeContainer>, std::placeholders::_1,
                      std::placeholders::_2, p_max_range)) {}
};

/**
 *
 * @brief   Distance metric factory provides services for creation available
 * metric in the 'pyclustering::utils::metric' and also user-defined.
 *
 */
template <typename TypeContainer>
class distance_metric_factory {
 public:
   ...
   *
   */
  static distance_metric<TypeContainer> gower(
      const TypeContainer& p_max_range) {
   ...
   *
   * @param[in] p_functor: user-defined metric for calculation distance between
   * two points.
   *
   * @return  User-defined distance metric.
   *
   */
  static distance_metric<TypeContainer> user_defined(
      const distance_functor<TypeContainer>& p_functor) {
...
/**
 *
 * @brief   Returns average distance for establish links between specified
 * number of neighbors.
 *
 * @param[in] points:    Input data.
 * @param[in] num_neigh: Number of neighbors.
 *
 * @return  Returns average distance for establish links between 'num_neigh' in
 * data set 'points'.
 *
 */
double average_neighbor_distance(
    const std::vector<std::vector<double> >* points,
    const std::size_t num_neigh);

/**
 *
 * @brief   Finds farthest distance between points in specified container
 * (data).
 *
 * @param[in] p_container: input data.
 * @param[in] p_metric: metric that is used for distance calculation between
 * points.
 *
 ...
 */
template <typename TypeContainer>
double farthest_distance(const TypeContainer& p_container,
...
/**
 *
 * @brief   Calculates distance matrix using points container using Euclidean
 * distance.
 *
 ...
 */
template <typename TypeContainer>
void distance_matrix(const TypeContainer& p_points,
                     const distance_metric<point>& p_metric,
                     TypeContainer& p_distance_matrix) {
  using TypeElement = typename TypeContainer::value_type;

  p_distance_matrix =
      TypeContainer(p_points.size(), TypeElement(p_points.size(), 0.0));
...
void distance_matrix(const TypeContainer& p_points,
                     TypeContainer& p_distance_matrix) {
  distance_matrix(p_points, distance_metric_factory<point>::euclidean(),
                  p_distance_matrix);
}
