...
struct hnn_parameters {
 public:
  ...
  double m_gNa =
      120.0 *
      (1.0 + 0.02 * m_nu); /* Maximal conductivity for sodium current     */
  double m_gK =
      36.0 *
      (1.0 + 0.02 * m_nu); /* Maximal conductivity for potassium current  */
  double m_gL =
      0.3 *
      (1.0 + 0.02 * m_nu); /* Maximal conductivity for leakage current    */
  ...
  double m_Vsyninh =
      -80.0; /* Synaptic reversal potential [mV] for inhibitory effects   */
  double m_Vsynexc =
      0.0; /* Synaptic reversal potential [mV] for exciting effects     */

  double m_alfa_inhibitory =
      6.0; /* Alfa-parameter for alfa-function for inhibitory effect    */
  double m_betta_inhibitory =
      0.3; /* Betta-parameter for alfa-function for inhibitory effect   */

  double m_alfa_excitatory =
      40.0; /* Alfa-parameter for alfa-function for excitatoty effect    */
  double m_betta_excitatory =
      2.0; /* Betta-parameter for alfa-function for excitatoty effect   */
  ...
  double m_deltah = 650.0; /* Period of time [ms] when high strength value of
                              synaptic connection exists from CN2 to PN */
  double m_threshold =
      -10; /* Threshold of the membrane potential that should exceeded by
              oscillator to be considered as an active */
...
struct basic_neuron_state {
 public:
  double m_membrane_potential =
      0.0; /* Membrane potential of cenral neuron (V)                */
  double m_active_cond_sodium =
      0.0; /* Activation conductance of the sodium channel (m)       */
  double m_inactive_cond_sodium =
      0.0; /* Inactivaton conductance of the sodium channel (h)      */
  double m_active_cond_potassium =
      0.0; /* Activaton conductance of the sodium channel (h)        */
...
class hhn_dynamic {
 ...
 public:
  ...
  using network_collector =
      std::unordered_map<hhn_dynamic::collect, bool, hhn_dynamic::collect_hash>;

  using network_dynamic =
      std::unordered_map<hhn_dynamic::collect, evolution_dynamic,
                         hhn_dynamic::collect_hash>;
  using network_dynamic_ptr = std::shared_ptr<network_dynamic>;

 private:
  network_collector m_enable = {{collect::MEMBRANE_POTENTIAL, true},
  ...
  network_dynamic_ptr m_peripheral_dynamic =
      std::make_shared<network_dynamic>();
 ...
 public:
  ...
  void store(const double p_time,
  ...
  double get_peripheral_value(const std::size_t p_iteration,
                              const std::size_t p_index,
                              const hhn_dynamic::collect p_type) const;

  double get_central_value(const std::size_t p_iteration,
                           const std::size_t p_index,
                           const hhn_dynamic::collect p_type) const;

 private:
  void get_collected_types(const bool p_enabled,
                           std::set<hhn_dynamic::collect>& p_types) const;

  void reserve_collection(const hhn_dynamic::collect p_state,
                          const std::size_t p_size);

  void store_membrane_potential(const std::vector<hhn_oscillator>& p_peripheral,
                                const std::vector<central_element>& p_central);

  void store_active_cond_sodium(const std::vector<hhn_oscillator>& p_peripheral,
                                const std::vector<central_element>& p_central);

  void store_inactive_cond_sodium(
      const std::vector<hhn_oscillator>& p_peripheral,
      const std::vector<central_element>& p_central);

  void store_active_cond_potassium(
      const std::vector<hhn_oscillator>& p_peripheral,
      const std::vector<central_element>& p_central);

  static void reserve_dynamic_collection(const hhn_dynamic::collect p_state,
                                         const std::size_t p_size,
...
class hhn_dynamic_reader {
 ...
 private:
  ...
  void extract_dynamic(const std::string& p_line, double& p_time,
                       std::vector<hhn_oscillator>& p_peripheral,
                       std::vector<central_element>& p_central);

  void extract_state(std::istringstream& p_stream,
                     basic_neuron_state& p_state) const;

  static void extract_size_header(const std::string& p_line,
                                  std::size_t& p_size_dynamic,
                                  std::size_t& p_size_network);

  static void extract_enable_header(
...
class hhn_network {
 ...
 public:
  ...
  hhn_network(const std::size_t p_size, const hnn_parameters& p_parameters);

  ~hhn_network() = default;

 public:
  void simulate(const std::size_t p_steps, const double p_time,
                const solve_type p_solver, const hhn_stimulus& p_stimulus,
 ...
 private:
  ...
  void calculate_states(const solve_type p_solver, const double p_time,
                        const double p_step, const double p_int_step);

  void calculate_peripheral_states(const solve_type p_solver,
                                   const double p_time, const double p_step,
                                   const double p_int_step,
                                   hhn_states& p_next_states);

  void calculate_central_states(const solve_type p_solver, const double p_time,
                                const double p_step, const double p_int_step,
                                hhn_states& p_next_states);

  void perform_calculation(const solve_type p_solver, const double p_time,
                           const double p_step, const double p_int_step,
                           const differ_state<double>& p_inputs,
                           const differ_extra<>& p_extra,
                           hhn_state& p_next_states);

  void neuron_states(const double t, const differ_state<double>& inputs,
  ...
  double peripheral_synaptic_current(const std::size_t p_index,
                                     const double p_time,
                                     const double p_membrane) const;

  double central_first_synaptic_current(const double p_time,
                                        const double p_membrane) const;
  ...
  void assign_neuron_states(const double p_time, const double p_step,
                            const hhn_states& p_next_peripheral,
                            const hhn_states& p_next_central);

  static double alpha_function(const double p_time, const double p_alfa,
                               const double p_betta);

  template <class NeuronType>
  static void pack_equation_input(const NeuronType& p_neuron,
                                  differ_state<double>& p_inputs);

  template <class NeuronType>
  static void unpack_equation_output(const hhn_state& p_outputs,
                                     NeuronType& p_neuron);
};

template <class NeuronType>
void hhn_network::pack_equation_input(const NeuronType& p_neuron,
                                      differ_state<double>& p_inputs) {
...
template <class NeuronType>
void hhn_network::unpack_equation_output(const hhn_state& p_outputs,
                                         NeuronType& p_neuron) {
  p_neuron.m_membrane_potential =
      p_outputs[0].state[POSITION_MEMBRAN_POTENTIAL];
  p_neuron.m_active_cond_sodium =
      p_outputs[0].state[POSITION_ACTIVE_COND_SODIUM];
  p_neuron.m_inactive_cond_sodium =
      p_outputs[0].state[POSITION_INACTIVE_COND_SODIUM];
  p_neuron.m_active_cond_potassium =
