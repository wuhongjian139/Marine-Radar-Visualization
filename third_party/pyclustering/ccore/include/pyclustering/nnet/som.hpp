...
/**
 *
 * @brief   Connection structures that can be established in self-organized
 * feature map.
 *
 */
enum class som_conn_type {
  ...
  /*!< Grid type of connections when each oscillator has connections with left,
     upper-left, upper, upper-right, right, right-lower, lower, lower-left
     neighbors. */
  SOM_GRID_EIGHT = 1,

  /*!< Grid type of connections when each oscillator has connections with left,
     upper-left, upper-right, right, right-lower, lower-left neighbors. */
  SOM_HONEYCOMB = 2,

  /*!< Grid type of connections when existance of each connection is defined by
     the SOM rule on each step of simulation. */
  SOM_FUNC_NEIGHBOR = 3
};
...
enum class som_init_type {
  ...
  /*!< Weights are randomly distributed using Gaussian distribution (input data
     centroid, 1). */
  SOM_RANDOM_CENTROID = 1,

  /*!< Weights are randomly distrbiuted using Gaussian distribution (input data
     centroid, surface of input data). */
  SOM_RANDOM_SURFACE = 2,

  /*!< Weights are distributed as a uniform grid that covers whole surface of
     the input data. */
  SOM_UNIFORM_GRID = 3
};
...
class som {
 ...
 public:
   ...
   * @param[in] conn_type: type of connection between oscillators in the
   * network.
   * @param[in] parameters: others parameters of the network.
   *
   */
  som(const std::size_t num_rows, const std::size_t num_cols,
      const som_conn_type type_conn, const som_parameters& parameters);
 ...
 public:
   ...
   *
   */
  std::size_t train(const dataset& input_data, const size_t num_epochs,
                    bool autostop);

  /**
   *
   * @brief   Initialize SOM network by loading weights.
   * @details This method is provided service to load trained network parameters
   * to avoid network training that may take a lot of time.
   *
   * @param[in] p_weights: neuron weights.
   * @param[in] p_awards: amount of captured objects by each neuron during
   * training (can be empty if it is not required).
   * @param[in] p_capture_objects: captured objects by each neuron during
   * training (can be empty if it is not required).
   *
   * @return  Returns number of learining iterations.
   *
   */
  void load(const dataset& p_weights, const som_award_sequence& p_awards,
            const som_gain_sequence& p_capture_objects);

  /**
   *
   * @brief   Processes input pattern (no learining) and returns index of
   * neuron-winner.
   * @details Using index of neuron winner catched object can be obtained by
   * get_capture_objects().
   *
   ...
   *
   */
  inline const dataset& get_weights() const { return m_weights; }

  /**
   *
   * @return  Constant reference to sequence of captured objects by each neuron
   * during training for read-only purposes.
   *
   */
  ...
  /**
   *
   * @return  Constant reference to neighbors of each neuron for read-only
   * purposes.
   *
   */
  ...
  /**
   *
   * @return  Constant reference to amount of captured objects by each neuron
   * during training for read-only purposes.
   *
   */
  inline const som_award_sequence& get_awards() const { return m_awards; }

  /**
   *
   * @return  Reference to amount of captured objects by each neuron during
   * training.
   *
   */
  inline som_award_sequence& get_awards() { return m_awards; }

 private:
  /**
   *
   * @brief   Create connections in line with input rule (grid four, grid eight,
  ...
  /**
   *
   * @brief   Creates initial weights for neurons in line with the specified
   * initialization.
   *
   ...
   *
   * @param[in] input_pattern: input pattern from the input data set, for
   * example it can be coordinates of point.
   *
   ...
   *
   */
  std::size_t adaptation(const size_t index_winner,
                         const pattern& input_pattern);

  /**
   *
   * @brief   Returns maximum changes of weight in line with comparison between
   * previous weights and current weights.
   *
   * @return  Returns value that represents maximum changes of weight after
   * adaptation process.
   *
   */
