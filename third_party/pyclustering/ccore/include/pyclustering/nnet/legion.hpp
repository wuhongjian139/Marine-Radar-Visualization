...
struct legion_network_state {
 ...
 public:
  ...
  explicit legion_network_state(const std::size_t size)
      : m_output(size, 0.0), m_inhibitor(0.0), m_time(0.0) {}
...
class legion_network {
 private:
  ...
  legion_stimulus *m_stimulus =
      nullptr; /* just keep it during simulation for convinience (pointer to
                  external object, legion is not owner) */
 ...
 public:
  ...
  legion_network(const size_t num_osc, const connection_t connection_type,
 ...
 public:
  void simulate(const unsigned int steps, const double time,
                const solve_type solver, const bool collect_dynamic,
 ...
 private:
  ...
  void calculate_states(const legion_stimulus &stimulus,
                        const solve_type solver, const double t,
                        const double step, const double int_step);

  void inhibitor_state(const double t, const differ_state<double> &inputs,
  ...
  void neuron_simplify_states(const double t,
                              const differ_state<double> &inputs,
                              const differ_extra<void *> &argv,
                              differ_state<double> &outputs);

  void store_dynamic(const double time, const bool collect_dynamic,
                     legion_dynamic &dynamic) const;

  void initialize(const size_t num_osc, const connection_t connection_type,
                  const size_t height, const size_t width,
                  const legion_parameters &params);
};
