...
class dynamic_analyser {
 ...
 public:
  ...
  dynamic_analyser(const double p_threshold,
                   const double p_tolerance = DEFAULT_TOLERANCE,
                   const std::size_t p_spikes = DEFAULT_AMOUNT_SPIKES);

  template <class DynamicType, class EnsemblesType>
  void allocate_sync_ensembles(
      const DynamicType& p_dynamic, EnsemblesType& p_ensembles,
      typename EnsemblesType::value_type& p_dead) const;

 private:
  template <class DynamicType>
  void extract_oscillations(
      const DynamicType& p_dynamic,
      std::vector<spike_collection>& p_oscillations) const;

  template <class DynamicType>
  void extract_spikes(const DynamicType& p_dynamic, const std::size_t p_index,
                      spike_collection& p_spikes) const;

  template <class EnsemblesType>
  void extract_ensembles(const std::vector<spike_collection>& p_oscillations,
                         EnsemblesType& p_ensembles,
                         typename EnsemblesType::value_type& p_dead) const;

  template <class DynamicType>
  std::size_t find_spike_end(const DynamicType& p_dynamic,
                             const std::size_t p_index,
                             const std::size_t p_position) const;

  bool is_sync_spikes(const spike_collection& p_spikes1,
                      const spike_collection& p_spikes2) const;
};

template <class DynamicType, class EnsemblesType>
void dynamic_analyser::allocate_sync_ensembles(
    const DynamicType& p_dynamic, EnsemblesType& p_ensembles,
    typename EnsemblesType::value_type& p_dead) const {
...
template <class DynamicType>
void dynamic_analyser::extract_oscillations(
    const DynamicType& p_dynamic,
    std::vector<spike_collection>& p_oscillations) const {
  ...
  for (std::size_t index_neuron = 0; index_neuron < amount_oscillators;
       index_neuron++) {
...
template <class DynamicType>
void dynamic_analyser::extract_spikes(const DynamicType& p_dynamic,
                                      const std::size_t p_index,
                                      spike_collection& p_spikes) const {
  std::size_t position = p_dynamic.size() - 1;
  for (std::size_t cur_spike = 0; (cur_spike < m_spikes) && (position > 0);
       cur_spike++) {
    ...
    for (position = stop;
         (position > 0) && (p_dynamic[position][p_index] >= m_threshold);
         position--) {
    }
...
template <class DynamicType>
std::size_t dynamic_analyser::find_spike_end(
    const DynamicType& p_dynamic, const std::size_t p_index,
    const std::size_t p_position) const {
  ...
  /* if active state is detected, it means we don't have whole oscillatory
   * period for the considered oscillator, should be skipped */
  if (spike_fired) {
    for (; (p_dynamic[time_stop_simulation][p_index] >= m_threshold) &&
           (time_stop_simulation > 0);
         time_stop_simulation--) {
    }
...
template <class EnsemblesType>
void dynamic_analyser::extract_ensembles(
    const std::vector<spike_collection>& p_oscillations,
    EnsemblesType& p_ensembles,
    typename EnsemblesType::value_type& p_dead) const {
  ...
  for (std::size_t index_neuron = 0; index_neuron < p_oscillations.size();
       index_neuron++) {
    ...
    for (auto& ensemble : p_ensembles) {
      ...
      const spike_collection& ensemble_anchor_spikes =
          p_oscillations[anchour_neuron_index];
