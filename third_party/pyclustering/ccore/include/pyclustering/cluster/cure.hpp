...
struct cure_cluster {
 ...
 public:
  ...
  /**
   *
   * @brief   Default constructor of cure cluster that corresponds to specified
   * point.
   *
   */
...
class cure_cluster_comparator {
 public:
  /**
   *
   * @brief   Defines compare procedure using closest distance without checking
   * for nullptr.
   *
   ...
   *
   */
  bool operator()(const cure_cluster *const obj1,
                  const cure_cluster *const obj2) const;
};

/**
 *
 * @brief   Cure sorted queue of cure clusters. Sorting is defined by distances
 * between clusters. First element is always occupied by cluster whose distance
 * to the neighbor cluster is the smallest in the queue.
 *
 */
class cure_queue {
 private:
 ...
 private:
  ...
  /**
   *
   * @brief   Remove cluster from sorted queue (it's not removing of new
   * object). Used by sorting procedures.
   *
   ...
   *
   * @return  Returns true if all the elements in the cluster were found to be
   * same.
   *
   */
 ...
 public:
  using iterator =
      std::multiset<cure_cluster *, cure_cluster_comparator>::iterator;
  using const_iterator =
      std::multiset<cure_cluster *, cure_cluster_comparator>::const_iterator;
  ...
  /**
   *
   * @brief   Default copy constructor of sorted queue of cure clusters is
   * forbidden.
   *
  ...
  /**
   *
   * @brief   Merge cure clusters in line with the rule of merging of cure
   * algorithm.
   *
   * @param[in|out] cluster1: pointer to cure cluster 1.
   * @param[in|out] cluster2: pointer to cure cluster 2.
   * @param[in] number_repr_points: number of representative points for merged
   * cluster.
   * @param[in] compression: level of compression for calculation representative
   * points.
   *
   */
  void merge(cure_cluster *cluster1, cure_cluster *cluster2,
             const size_t number_repr_points, const double compression);
  ...
  /**
   *
   * @brief   Returns iterator to the end of CURE cluster collection (not a last
   * element).
   *
   */
  ...
  /**
   *
   * @brief   Returns constant iterator to the end of CURE cluster collection
   * (not a last element).
   *
   */
...
class relocation_info {
 ...
 public:
  relocation_info(const cure_queue::iterator &cluster_iterator,
                  cure_cluster *closest_cluster, const double closest_distance);
...
class cure : public cluster_algorithm {
 ...
 public:
   ...
   * @param[in] level_compression: level of compression for calculation new
   * representative points for merged cluster.
   *
   */
  cure(const size_t clusters_number, const size_t points_number,
