...
struct optics_descriptor {
 ...
 public:
   ...
   * @details Processing is always false after creating for any created optics
   * descriptor.
   *
   * @param[in] p_index: index of optics object that corresponds to index of
   * real object in dataset.
   ...
   *
   */
  optics_descriptor(const std::size_t p_index, const double p_core_distance,
                    const double p_reachability_distance);
 ...
 public:
  /**
   *
   * @brief Clears core and reachability distances and processing flag (at the
   * same time index is not reseted).
   *
   */
  void clear();
};
...
/**
 *
 * @brief Less comparator for object description that used by OPTICS algorithm
...
struct optics_pointer_descriptor_less {
  /**
   *
   * @brief Compare two OPTICS object using following rule: p_object1 <
   * p_object2.
   *
   ...
   *
   */
  bool operator()(const optics_descriptor* p_object1,
                  const optics_descriptor* p_object2) const;
};
