...
class elbow {
  ...
  void process(const dataset& p_data, elbow_data& p_result) {
    if (p_data.size() < m_kmax) {
      throw std::invalid_argument("K max value '" + std::to_string(m_kmax) +
                                  "' is greater than amount of data points '" +
    ...
    parallel_for(m_kmin, m_kmax,
                 [this](const std::size_t p_index) { calculate_wce(p_index); });
 ...
 private:
  ...
  typename std::enable_if<
      std::is_same<CenterInitializer, kmeans_plus_plus>::value,
      void>::type static prepare_centers(const std::size_t p_amount,
                                         const dataset& p_data,
                                         dataset& p_initial_centers) {
    kmeans_plus_plus(p_amount, kmeans_plus_plus::FARTHEST_CENTER_CANDIDATE)
        .initialize(p_data, p_initial_centers);
  }
  ...
  void verify() {
    if (m_kmax < 3 + m_kmin) {
      throw std::invalid_argument("Amount of K '" +
                                  std::to_string(m_kmax - m_kmin) +
  ...
  void calculate_elbows() {
    ...
    for (std::size_t index_elbow = 1;
         index_elbow < m_result->get_wce().size() - 1; index_elbow++) {
      const double x = static_cast<double>(index_elbow);
      const double y = wce.at(index_elbow);

      const double segment =
          std::abs((y0 - y1) * x + (x1 - x0) * y + (x0 * y1 - x1 * y0));
  ...
  std::size_t find_optimal_kvalue() {
    auto optimal_elbow_iter =
        std::max_element(m_elbow.cbegin(), m_elbow.cend());
