...
class hsyncnet : public syncnet {
 ...
 public:
  hsyncnet(std::vector<std::vector<double> >* input_data,
           const std::size_t cluster_number, const initial_type initial_phases);

  hsyncnet(std::vector<std::vector<double> >* input_data,
           const std::size_t cluster_number, const initial_type initial_phases,
           const std::size_t initial_neighbors, const double increase_persent);
 ...
 public:
  virtual void process(const double order, const solve_type solver,
                       ...
                       hsyncnet_analyser& analyser) override;

 private:
  void store_state(sync_network_state& state, hsyncnet_analyser& analyser);

  double calculate_radius(const double radius,
