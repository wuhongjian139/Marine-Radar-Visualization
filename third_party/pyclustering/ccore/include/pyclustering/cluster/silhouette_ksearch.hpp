...
class silhouette_ksearch_allocator {
 ...
 public:
  virtual void allocate(const std::size_t p_amount, const dataset& p_data,
                        cluster_sequence& p_clusters) = 0;
};

class kmeans_allocator : public silhouette_ksearch_allocator {
 public:
  virtual void allocate(const std::size_t p_amount, const dataset& p_data,
                        cluster_sequence& p_clusters) override;
};
...
class silhouette_ksearch {
 private:
  std::size_t m_kmin;
  std::size_t m_kmax;
  silhouette_ksearch_allocator::ptr m_allocator =
      std::make_shared<kmeans_allocator>();

 public:
  silhouette_ksearch() = default;

  silhouette_ksearch(const std::size_t p_kmin, const std::size_t p_kmax,
