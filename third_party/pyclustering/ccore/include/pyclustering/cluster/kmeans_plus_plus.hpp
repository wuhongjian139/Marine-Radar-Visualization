...
class kmeans_plus_plus : public center_initializer {
 public:
  /**
   *
   * @brief Denotes that the farthest center candidate (with highest
   * probability) should be used as a center.
   *
   */
 ...
 public:
   ...
   * @param[in] p_candidates: amount of candidates that are considered to find
   * the best center, if the farthest candidate is required (with highest
   * probability) than static constant FARTHEST_CENTER_CANDIDATE can be
   * specified.
   *
  ...
  kmeans_plus_plus(const std::size_t p_amount,
                   const std::size_t p_candidates = 1) noexcept;
   ...
   *
   */
  kmeans_plus_plus(const std::size_t p_amount, const std::size_t p_candidates,
                   const metric &p_metric) noexcept;
 ...
 public:
  /**
   *
   * @brief    Performs center initialization process in line algorithm
   * configuration.
   *
   ...
   *
   * @brief    Performs center initialization process in line algorithm
   * configuration for specific range of points.
   *
   * @param[in]  p_data: data for that centers are calculated.
   * @param[in]  p_indexes: point indexes from data that are defines which
   * points should be considered during calculation process. If empty then all
   * data points are considered.
   * @param[out] p_centers: initialized centers for the specified data.
   *
   */
  void initialize(const dataset &p_data, const index_sequence &p_indexes,
                  dataset &p_centers) const override;

  /**
   *
   * @brief    Performs center initialization process in line algorithm
   * configuration using real points from dataset as centers.
   *
   * @param[in]  p_data: data for that centers are calculated.
   * @param[out] p_center_indexes: initialized center indexes for the specified
   * data where indexes correspond to points from the data.
   *
   */
  void initialize(const dataset &p_data,
                  index_sequence &p_center_indexes) const;
 ...
 private:
   ...
   * points should be considered during calculation process. If empty then all
   * data points are considered.
   * @param[out] p_proc: function that defines how to store output result of the
   * algorithm.
   *
   */
  void initialize(const dataset &p_data, const index_sequence &p_indexes,
                  const store_result &p_proc) const;

  /**
   *
   * @brief    Store obtained center.
   *
   * @param[in]  p_proc: function that defines how to store output result of the
   * algorithm.
   * @param[in]  p_result: initialized center that should be stored.
   *
   */
  void store_center(const store_result &p_proc,
                    center_description &p_result) const;

  /**
   *
   * @brief    Store pointers to data, indexes and centers to avoiding passing
   * them between class methods.
   * @details  Pointers are reseted when center initialization is over.
   *
   * @param[in]  p_data: data for that centers are calculated.
   * @param[in]  p_indexes: point indexes from data that are defines which
   * points should be considered during calculation process.
   *
   * @return   The first initialized center.
   *
   */
  void store_temporal_params(const dataset &p_data,
  ...
  /**
   *
   * @brief    Calculates the next most probable center in line with weighted
   * distribution.
   *
  ...
  /**
   *
   * @brief    Calculates distance from the specified point to the closest
   * center.
   *
  ...
  /**
   *
   * @brief    Calculates center probability for each point using distances to
   * closest centers.
   *
   ...
   *
   */
  void calculate_probabilities(const std::vector<double> &p_distances,
                               std::vector<double> &p_probabilities) const;
   ...
   *
   */
  std::size_t get_probable_center(
      const std::vector<double> &p_distances,
      const std::vector<double> &p_probabilities) const;
};
