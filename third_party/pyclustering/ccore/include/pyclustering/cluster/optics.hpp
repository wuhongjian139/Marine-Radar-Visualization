 ...
 *
 */
enum class optics_data_t { POINTS, DISTANCE_MATRIX };

/**
 *
 * @brief Class represents clustering algorithm OPTICS (Ordering Points To
 * Identify Clustering Structure).
 * @details OPTICS is a density-based algorithm. Purpose of the algorithm is to
 * provide explicit clusters, but create clustering-ordering representation of
 * the input data. Clustering-ordering information contains information about
 * internal structures of data set in terms of density and proper connectivity
 * radius can be obtained for allocation required amount of clusters using this
 * diagram. In case of usage additional input parameter 'amount of clusters'
 * connectivity radius should be bigger than real - because it will be
...
class optics : public cluster_algorithm {
 ...
 private:
  ...
  struct neighbor_descriptor_less {
   public:
    bool operator()(const neighbor_descriptor &p_object1,
                    const neighbor_descriptor &p_object2) const {
      return p_object1.m_reachability_distance <
             p_object2.m_reachability_distance;
    }
  };

  using neighbors_collection =
      std::multiset<neighbor_descriptor, neighbor_descriptor_less>;
 ...
 public:
   ...
   * @param[in] p_neighbors: minimum amount of shared neighbors that is require
   * to connect two object (if distance between them is less than connectivity
   * radius).
   *
   */
   ...
   * (in this case connectivity radius may be changed by the algorithm.
   */
  optics(const double p_radius, const std::size_t p_neighbors,
 ...
 public:
   ...
   * @param[out] p_result: clustering result of an input data (consists of
   * allocated clusters, cluster-ordering, noise and proper connectivity
   * radius).
   *
   */
  ...
  /**
   *
   * @brief    Performs cluster analysis of specific input data (points or
   * distance matrix) that is defined by the 'p_type' argument.
   *
   * @param[in]  p_data: input data for cluster analysis.
   ...
   *
   */
  virtual void process(const dataset &p_data, const optics_data_t p_type,
 ...
 private:
  ...
  void get_neighbors(const std::size_t p_index,
                     neighbors_collection &p_neighbors);

  void get_neighbors_from_points(const std::size_t p_index,
                                 neighbors_collection &p_neighbors);

  void get_neighbors_from_distance_matrix(const std::size_t p_index,
                                          neighbors_collection &p_neighbors);

  double get_core_distance(const neighbors_collection &p_neighbors) const;

  void update_order_seed(
      const optics_descriptor &p_object,
      const neighbors_collection &p_neighbors,
