...
enum class dbscan_data_t { POINTS, DISTANCE_MATRIX };
...
class dbscan {
 private:
  ...
  dbscan_data* m_result_ptr = nullptr; /* temporary pointer to clustering result
                                          that is used only during processing */
  ...
  double m_initial_radius =
      0.0; /* original radius that was specified by user */
 ...
 public:
   ...
   * @param[in] p_minimum_neighbors: minimum amount of shared neighbors that is
   * require to connect two object (if distance between them is less than
   * connectivity radius).
   *
   */
 ...
 public:
   ...
   *
   */
  virtual void process(const dataset& p_data, const dbscan_data_t p_type,
 ...
 private:
   ...
   * @param[out] p_neighbors: neighbor indexes of the specified node (data
   * object).
   *
   */
  void get_neighbors(const size_t p_index, std::vector<size_t>& p_neighbors);

  void get_neighbors_from_points(const size_t p_index,
                                 std::vector<size_t>& p_neighbors);

  void get_neighbors_from_distance_matrix(const size_t p_index,
                                          std::vector<size_t>& p_neighbors);
