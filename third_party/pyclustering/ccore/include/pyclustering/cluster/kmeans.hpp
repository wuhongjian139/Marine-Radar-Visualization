...
class kmeans : public cluster_algorithm {
 ...
 private:
  ...
  const index_sequence* m_ptr_indexes =
      nullptr; /* temporary pointer to indexes */
 ...
 public:
   ...
   *
   * @param[in]     p_data: input data for cluster analysis.
   * @param[in]     p_indexes: specify indexes of objects in 'p_data' that
   * should be used during clustering process.
   * @param[in|out] p_result: clustering result of an input data, it is also
   * considered as an input argument to where observer parameter can be set to
   * collect changes of clusters and centers on each step of processing.
   *
   */
  virtual void process(const dataset& p_data, const index_sequence& p_indexes,
 ...
 private:
  ...
  void assign_point_to_cluster(const std::size_t p_index_point,
  ...
  /**
   *
   * @brief    Calculates total within-cluster errors that is based on distance
   * metric.
   *
   */
