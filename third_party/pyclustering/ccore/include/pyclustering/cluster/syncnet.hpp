...
@brief   Analyser for syncnet - oscillatory neural network based on Kuramoto
...
@brief   Oscillatory neural network based on Kuramoto model for cluster
analysis.
...
class syncnet : public sync_network {
 ...
 public:
  ...
  @brief   Contructor of the adapted oscillatory network SYNC for cluster
  analysis.

  @param[in] input_data: input data for clustering.
  @param[in] connectivity_radius: connectivity radius between points.
  @param[in] enable_conn_weight: if True - enable mode when strength between
  oscillators depends on distance between two oscillators. Otherwise all
  connection between oscillators have the same strength.
  @param[in] initial_phases: type of initialization of initial phases of
  oscillators.

  */
  syncnet(std::vector<std::vector<double> > *input_data,
          const double connectivity_radius, const bool enable_conn_weight,
  ...
  */
  ...
  @brief Network is trained via achievement sync state between the oscillators
  using the radius of coupling.
  ...
  @param[in]  collect_dynamic: specified requirement to collect whole dynamic of
  the network.
  @param[out] analyser: analyser of sync results of clustering.

  */
  virtual void process(const double order, const solve_type solver,
                       const bool collect_dynamic, syncnet_analyser &analyser);
 ...
 protected:
  ...
  @brief   Create connections between oscillators in line with input radius of
  connectivity.
