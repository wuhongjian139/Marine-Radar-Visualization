...
class rock : public cluster_algorithm {
 ...
 public:
  ...
  /**
   *
   * @brief    Creates ROCK solver in line with specified parameters of the
   * algorithm.
   *
   * @param[in] radius: connectivity radius (similarity threshold), points are
   * neighbors if distance between them is less than connectivity radius.
   * @param[in] number_clusters: amount of clusters that should be allocated.
   * @param[in] threshold: defines degree of normalization that influences on
   * choice of clusters for merging during processing.
   *
   */
  rock(const double radius, const std::size_t number_clusters,
 ...
 private:
  /**
   *
   * @brief    Creates adjacency matrix where each element described existence
   * of link between points (means that points are neighbors).
   *
   * @param[in]  p_data: input data for cluster analysis.
   *
   */
  ...
  /**
   *
   * @brief    Finds two clusters that are most suitable candidates for merging
   * and than merges them.
   *
   */
  bool merge_cluster();
   ...
   * @details  Link between objects (points) exists only if distance between
   * them less than connectivity radius.
   *
   ...
   *
   */
  size_t calculate_links(const cluster& cluster1,
                         const cluster& cluster2) const;

  /**
   *
   * @brief    Calculates coefficient 'goodness measurement' between two
   * clusters.
   * @details  The coefficient defines level of suitability of clusters for
   * merging.
   *
   * @param[in] cluster1: the first cluster.
   * @param[in] cluster2: the second cluster.
   ...
   *
   */
  double calculate_goodness(const cluster& cluster1,
                            const cluster& cluster2) const;
};
