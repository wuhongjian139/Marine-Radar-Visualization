...
enum class kmedoids_data_t { POINTS, DISTANCE_MATRIX };

/**
 *
 * @brief    Represents K-Medoids clustering algorithm for cluster analysis.
 * @details  The algorithm related to partitional class when input data is
 * divided into groups.
 *
 */
class kmedoids : public cluster_algorithm {
 ...
 private:
  using distance_calculator =
      std::function<double(const std::size_t, const std::size_t)>;

 private:
  const dataset* m_data_ptr = nullptr; /* temporary pointer to input data that
                                          is used only during processing */

  kmedoids_data* m_result_ptr =
      nullptr; /* temporary pointer to clustering result that is used only
                  during processing */
 ...
 public:
   ...
   * of distance change of medoids of clusters is less than tolerance than
   * algorithm will stop processing.
   * @param[in] p_itermax: maximum amount of iterations (by default
   * kmedoids::DEFAULT_ITERMAX).
 ...
 public:
   ...
   *
   */
  virtual void process(const dataset& p_data, const kmedoids_data_t p_type,
 ...
 private:
  ...
  /**
   *
   * @brief    Calculates maximum difference in data allocation between previous
   * medoids and specified.
   *
   * @param[in] p_medoids: medoids that should be used for difference
   * calculation.
   *
  ...
  /**
   *
   * @brief    Creates distance calcultor in line with data type and distance
   * metric metric.
   *
   * @param[in] p_type: data type (points or distance matrix).
   *
   ...
   *
   * @return   Index of cluster that is appropriate for the particular point. If
   * point is a medoid then OBJECT_ALREADY_CONTAINED value is returned.
   *
   */
  std::size_t find_appropriate_cluster(const std::size_t p_index,
                                       medoid_sequence& p_medoids);
};
