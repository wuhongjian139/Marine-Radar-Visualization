...
class kmedians : public cluster_algorithm {
 ...
 private:
  ...
  kmedians_data* m_ptr_result =
      nullptr; /* temporary pointer to output result */
 ...
 public:
   ...
   * of distance change of medians of clusters is less than tolerance than
   * algorithm will stop processing.
   * @param[in] p_max_iter: maximum amount of iteration for clustering.
   * @param[in] p_metric: distance metric for distance calculation between
   * objects.
   *
   */
 ...
 private:
  ...
  /**
   *
   * @brief    Assign point to cluster by marking corresponding index in
   * container 'p_lables'.
   *
   * @param[in] p_index_point: index of point that should be assigned to
   * cluster.
   * @param[in] p_medians: medians that corresponds to clusters.
   * @param[out] p_lables: cluster labels for each point (cluster labels has the
   * same size as an input data).
   *
   */
   ...
   *
   * @param[in|out] clusters: clusters that are sorted and used for updating
   * medians.
   ...
   *
   * @param[in|out] current_cluster: cluster that is sorted and used for
   * updating medians.
