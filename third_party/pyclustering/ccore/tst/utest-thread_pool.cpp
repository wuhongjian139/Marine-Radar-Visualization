...
static void template_add_task_test(const std::size_t p_pool_size,
                                   const std::size_t p_task_amount) {
  ...
  for (std::size_t task_index = 0; task_index < p_task_amount; task_index++) {
    ...
    ASSERT_EQ(
        std::find(task_storage.cbegin(), task_storage.cend(), client_task),
        task_storage.cend());
...
TEST(utest_thread_pool, pool_1_thread_1_task) { template_add_task_test(1, 1); }

TEST(utest_thread_pool, pool_1_thread_2_tasks) { template_add_task_test(1, 2); }

TEST(utest_thread_pool, pool_1_thread_3_tasks) { template_add_task_test(1, 3); }
