...
TEST(utest_interface_metric, euclidean) {
  ...
  void *metric_pointer =
      metric_create(metric_t::EUCLIDEAN, arguments.get(), p_solver);
  ...
  double distance =
      metric_calculate(metric_pointer, point1.get(), point2.get());
...
TEST(utest_interface_metric, gower) {
  std::shared_ptr<pyclustering_package> arguments =
      pack(std::vector<double>({1.0, 0.0}));
  double (*p_solver)(const void *, const void *) = nullptr;

  void *metric_pointer =
      metric_create(metric_t::GOWER, arguments.get(), p_solver);
