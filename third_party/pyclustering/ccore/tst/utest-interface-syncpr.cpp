...
TEST(utest_interface_syncpr, syncpr_api) {
  ...
  std::shared_ptr<pyclustering_package> patterns =
      pack(std::vector<syncpr_pattern>({{1, 1, -1, 1, 1, -1, 1, 1, -1}}));
  syncpr_train(network_pointer, patterns.get());

  std::shared_ptr<pyclustering_package> pattern =
      pack(syncpr_pattern({1, 1, -1, 1, 1, -1, 1, 1, -1}));
  void* dynamic1 =
      syncpr_simulate_static(network_pointer, 10, 10, pattern.get(),
                             (unsigned)solve_type::FORWARD_EULER, true);
  ASSERT_NE(nullptr, dynamic1);

  void* dynamic2 =
      syncpr_simulate_dynamic(network_pointer, pattern.get(), 0.95,
                              (unsigned)solve_type::FORWARD_EULER, true, 0.1);
  ...
  pyclustering_package* package =
      syncpr_dynamic_allocate_sync_ensembles(dynamic1, 0.1);
