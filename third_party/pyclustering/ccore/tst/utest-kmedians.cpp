...
static void template_kmedians_length_process_data(
    const dataset_ptr& p_data, const dataset& p_start_medians,
  ...
  kmedians solver(p_start_medians, kmedians::DEFAULT_TOLERANCE, p_itermax,
                  p_metric);
...
TEST(utest_kmedians, allocation_sample_simple_01) {
  ...
  template_kmedians_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_medians, expected_clusters_length);
}
...
TEST(utest_kmedians, allocation_sample_simple_01_euclidean) {
  ...
  template_kmedians_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_medians, expected_clusters_length, kmedians::DEFAULT_ITERMAX,
...
TEST(utest_kmedians, allocation_sample_simple_03_hanging) {
  dataset start_medians = {
      {1.80508, 4.609467}, {0.926445, 0.126412}, {0.144706, 0.987019}};
  template_kmedians_length_process_data(
...
TEST(utest_kmedians, large_number_medians_sample_simple_02) {
  dataset start_medians = {{-1.5, 0.8},  {-4.9, 5.0}, {2.3, 3.2},
...
TEST(utest_kmedians, large_number_medians_sample_simple_03) {
  dataset start_medians = {{-8.1, 2.3}, {-4.9, 5.5}, {1.3, 8.3}, {-2.6, -1.7},
...
TEST(utest_kmedians, rough_medians_sample_simple_10) {
  dataset start_medians = {{0.0772944481804071, 0.05224990900863469},
                           {1.6021689021213712, 1.0347579135245601},
                           {2.3341008076636096, 1.280022869739064}};
  std::vector<size_t> expected_clusters_length; /* pass empty */
  template_kmedians_length_process_data(
...
TEST(utest_kmedians, itermax_0) {
  ...
  template_kmedians_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_medians, expected_clusters_length, 0);
}
...
TEST(utest_kmedians, itermax_1) {
  ...
  template_kmedians_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_medians, expected_clusters_length, 1);
}
...
TEST(utest_kmedians, itermax_10_simple01) {
  ...
  template_kmedians_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_medians, expected_clusters_length, 10);
}
...
TEST(utest_kmedians, itermax_10_simple02) {
  ...
  template_kmedians_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_02),
      start_medians, expected_clusters_length, 20);
}