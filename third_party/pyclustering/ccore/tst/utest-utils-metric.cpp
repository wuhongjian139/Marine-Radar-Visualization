...
TEST(utest_metric, metric_factory_gower) {
  distance_metric<point> metric =
      distance_metric_factory<point>::gower({0.0, 0.0});
  ASSERT_EQ(0.0, metric({0.0, 0.0}, {0.0, 0.0}));
...
TEST(utest_metric, metric_factory_user_defined) {
  distance_metric<point> metric = distance_metric_factory<point>::user_defined(
      [](const point& p1, const point& p2) { return -5.0; });
...
TEST(utest_metric, calculate_distance_matrix_01) {
  ...
  dataset distance_matrix_expected = {
