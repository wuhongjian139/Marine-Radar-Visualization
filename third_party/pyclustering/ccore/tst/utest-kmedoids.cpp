...
static void template_kmedoids_length_process_data(
    const dataset_ptr p_data, const medoid_sequence& p_start_medoids,
  ...
  kmedoids solver(p_start_medoids, kmedoids::DEFAULT_TOLERANCE, p_itermax,
                  p_metric);
...
static void template_kmedoids_length_process_distance_matrix(
    const dataset_ptr p_data, const medoid_sequence& p_start_medoids,
    const std::vector<size_t>& p_expected_cluster_length,
    const std::size_t p_itermax = kmedoids::DEFAULT_ITERMAX,
    const distance_metric<point>& p_metric =
...
TEST(utest_kmedoids, allocation_sample_simple_01_euclidean) {
  ...
  template_kmedoids_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_medoids, expected_clusters_length, kmedoids::DEFAULT_ITERMAX,
      distance_metric_factory<point>::euclidean());
}
...
TEST(utest_kmedoids,
     allocation_sample_simple_01_euclidean_square_distance_matrix) {
  const medoid_sequence start_medoids = {1, 5};
  const std::vector<size_t> expected_clusters_length = {5, 5};
  template_kmedoids_length_process_distance_matrix(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
...
TEST(utest_kmedoids, allocation_sample_simple_01_manhattan) {
  ...
  template_kmedoids_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_medoids, expected_clusters_length, kmedoids::DEFAULT_ITERMAX,
      distance_metric_factory<point>::manhattan());
}
...
TEST(utest_kmedoids, allocation_sample_simple_01_chebyshev) {
  ...
  template_kmedoids_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_medoids, expected_clusters_length, kmedoids::DEFAULT_ITERMAX,
      distance_metric_factory<point>::chebyshev());
}
...
TEST(utest_kmedoids, allocation_sample_simple_01_minkowski) {
  ...
  template_kmedoids_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_medoids, expected_clusters_length, kmedoids::DEFAULT_ITERMAX,
      distance_metric_factory<point>::minkowski(2.0));
}
...
TEST(utest_kmedoids, allocation_sample_simple_01_user_defined) {
  ...
  auto user_metric = [](const point& p1, const point& p2) {
  ...
  template_kmedoids_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_medoids, expected_clusters_length, kmedoids::DEFAULT_ITERMAX,
      distance_metric_factory<point>::user_defined(user_metric));
}
...
TEST(utest_kmedoids,
     allocation_sample_one_allocation_simple_01_distance_matrix) {
  const medoid_sequence start_medoids = {1};
  const std::vector<size_t> expected_clusters_length = {10};
  template_kmedoids_length_process_distance_matrix(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
...
TEST(utest_kmedoids,
     allocation_one_allocation_sample_simple_02_distance_matrix) {
  const medoid_sequence start_medoids = {10};
  const std::vector<size_t> expected_clusters_length = {23};
  template_kmedoids_length_process_distance_matrix(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_02),
...
TEST(utest_kmedoids, allocation_wrong_initial_medoids_sample_simple_03) {
  ...
  const std::vector<size_t>
      expected_clusters_length; /* empty - just check index point existence */
  template_kmedoids_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_03),
      start_medoids, expected_clusters_length);
}
...
TEST(utest_kmedoids, totally_similar_data) {
  ...
  template_kmedoids_length_process_data(dataset, start_medoids,
...
TEST(utest_kmedoids, itermax_0) {
  ...
  template_kmedoids_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_medoids, expected_clusters_length, 0);
}
...
TEST(utest_kmedoids, itermax_1) {
  ...
  template_kmedoids_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_medoids, expected_clusters_length, 1);
}
...
TEST(utest_kmedoids, itermax_10_simple01) {
  ...
  template_kmedoids_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_medoids, expected_clusters_length, 10);
}
...
TEST(performance_kmedoids, big_data) {
  ...
  auto points = simple_sample_factory::create_random_sample(cluster_length,
                                                            amount_clusters);

  medoid_sequence start_medoids = {10,
                                   cluster_length,
                                   cluster_length * 2,
                                   cluster_length * 3,
                                   cluster_length * 4,
