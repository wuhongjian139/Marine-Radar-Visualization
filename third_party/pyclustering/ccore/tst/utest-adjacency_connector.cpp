...
static void template_grid_connections(const grid_t p_connections,
  ...
  for (int index = 0; index < (int)collection.size(); index++) {
    ...
    if (upper_index >= 0) {
      ...
      ASSERT_TRUE(neighbors.cend() != std::find(neighbors.cbegin(),
                                                neighbors.cend(),
                                                (size_t)upper_index));
    }
    ...
    if ((right_index < (int)collection.size()) &&
        (std::ceil(right_index / base) == node_row_index)) {
    ...
    if (p_connections == grid_t::GRID_EIGHT) {
      if ((upper_left_index >= 0) &&
          (std::floor(upper_left_index / base) == upper_row_index)) {
        ASSERT_TRUE(collection.has_connection(index, upper_left_index));
        ASSERT_TRUE(neighbors.cend() != std::find(neighbors.cbegin(),
                                                  neighbors.cend(),
                                                  (size_t)upper_left_index));
      }

      if ((upper_right_index >= 0) &&
          (std::floor(upper_right_index / base) == upper_row_index)) {
        ASSERT_TRUE(collection.has_connection(index, upper_right_index));
        ASSERT_TRUE(neighbors.cend() != std::find(neighbors.cbegin(),
                                                  neighbors.cend(),
                                                  (size_t)upper_right_index));
      }

      if ((lower_left_index < (int)collection.size()) &&
          (std::floor(lower_left_index / base) == lower_row_index)) {
        ASSERT_TRUE(collection.has_connection(index, lower_left_index));
        ASSERT_TRUE(neighbors.cend() != std::find(neighbors.cbegin(),
                                                  neighbors.cend(),
                                                  (size_t)lower_left_index));
      }

      if ((lower_right_index < (int)collection.size()) &&
          (std::floor(lower_right_index / base) == lower_row_index)) {
        ASSERT_TRUE(collection.has_connection(index, lower_right_index));
        ASSERT_TRUE(neighbors.cend() != std::find(neighbors.cbegin(),
                                                  neighbors.cend(),
                                                  (size_t)lower_right_index));
      }

      for (int j = 0; j < (int)collection.size(); j++) {
        if ((j != index) && (j != upper_index) && (j != lower_index) &&
            (j != left_index) && (j != right_index) &&
            (j != upper_left_index) && (j != upper_right_index) &&
            (j != lower_left_index) && (j != lower_right_index)) {
...
TEST(utest_adjacency_connector, grid_eight_rectangle_10) {
  ...
  template_grid_connections<adjacency_weight_list>(grid_t::GRID_EIGHT, 10, 5,
                                                   2);
}
...
TEST(utest_adjacency_connector, grid_four_rectangle_50) {
  ...
  template_grid_connections<adjacency_weight_list>(grid_t::GRID_FOUR, 50, 5,
                                                   10);
}

TEST(utest_adjacency_connector, grid_eight_rectangle_50) {
  template_grid_connections<adjacency_bit_matrix>(grid_t::GRID_EIGHT, 50, 5,
                                                  10);
  ...
  template_grid_connections<adjacency_weight_list>(grid_t::GRID_EIGHT, 50, 5,
                                                   10);
}
...
TEST(utest_adjacency_connector, grid_four_rectangle_as_list) {
  ...
  template_grid_connections<adjacency_weight_list>(grid_t::GRID_FOUR, 15, 1,
                                                   15);
...
TEST(utest_adjacency_connector, grid_eight_rectangle_as_list) {
  template_grid_connections<adjacency_bit_matrix>(grid_t::GRID_EIGHT, 15, 1,
                                                  15);
  ...
  template_grid_connections<adjacency_weight_list>(grid_t::GRID_EIGHT, 15, 1,
                                                   15);
  ...
  for (size_t i = 0; i < p_elements; i++) {
    ...
    for (std::size_t j = 0; j < p_elements; j++) {
      if ((i != j) && (j != (i + 1)) && (j != (i - 1))) {
        ...
        ASSERT_TRUE(neighbors.cend() ==
                    std::find(neighbors.cbegin(), neighbors.cend(), (size_t)j));
      }
    }
  }
}
