...
template <class TypeContainer>
static void template_two_dimension_pyclustering_package(
...
TEST(utest_pyclustering, package_two_dimension_double) {
  std::vector<std::vector<double>> container = {
      {1.0, 2.5, 3.0}, {4.5, 5.5}, {6.1, 7.2, 8.3, 9.4}};
  template_two_dimension_pyclustering_package(container);
}
...
TEST(utest_pyclustering, package_unpack_two_dimension) {
  template_pack_unpack(
      std::vector<std::vector<double>>({{1.2, 2.4}, {3.6, 4.8, 5.0}, {6.0}}));
}