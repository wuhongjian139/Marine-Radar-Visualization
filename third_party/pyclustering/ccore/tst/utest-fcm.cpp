...
static void template_fcm_data_processing(
    const dataset_ptr& p_data, const dataset& p_start_centers, const double p_m,
  ...
  for (const auto& probablities : output_result.membership()) {
    double total_probability =
        std::accumulate(probablities.begin(), probablities.end(), 0.0);
    ASSERT_DOUBLE_EQ(1.0, total_probability);
  }

  ASSERT_CLUSTER_SIZES(*p_data, output_result.clusters(),
...
TEST(utest_fcm, allocation_sample_simple_01_hyper_4) {
  ...
  template_fcm_data_processing(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_centers, 4, expected_clusters_length);
}
...
TEST(utest_fcm, allocation_sample_simple_02_centers_are_points) {
  dataset start_centers = {
      {3.889032, 4.103663}, {7.090406, 6.647416}, {7.482343, 0.762685}};
  std::vector<size_t> expected_clusters_length = {10, 5, 8};
  template_fcm_data_processing(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_02),
      start_centers, 2, expected_clusters_length);
...
TEST(utest_fcm, allocation_famous_old_faithful) {
  ...
  template_fcm_data_processing(
      famous_sample_factory::create_sample(FAMOUS_SAMPLE::OLD_FAITHFUL),
      start_centers, 2, expected_clusters_length);
}

TEST(utest_fcm, allocation_fcps_hepta) {
  dataset start_centers = {{-0.06, 0.02, 0.02},  {2.41, 0.49, 0.03},
                           {-2.69, 0.34, 0.29},  {0.49, 2.89, 0.78},
                           {-0.60, -2.31, 0.05}, {-0.15, 0.77, 3.23},
                           {-0.50, 0.43, -2.60}};
  std::vector<size_t> expected_clusters_length = {30, 30, 30, 30, 30, 30, 32};
  template_fcm_data_processing(
      fcps_sample_factory::create_sample(FCPS_SAMPLE::HEPTA), start_centers, 2,
      expected_clusters_length);
}

TEST(utest_fcm, allocation_fcps_tetra) {
  dataset start_centers = {{1.001, -0.083, -0.681},
                           {-0.811, 0.476, -0.759},
                           {-0.956, -1.427, -0.020},
                           {0.225, 0.560, 1.794}};
  std::vector<size_t> expected_clusters_length = {100, 100, 100, 100};
  template_fcm_data_processing(
      fcps_sample_factory::create_sample(FCPS_SAMPLE::TETRA), start_centers, 2,
      expected_clusters_length);
}
...
TEST(utest_fcm, itermax_0) {
  ...
  template_fcm_data_processing(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_centers, 2, expected_clusters_length, 0);
}
...
TEST(utest_fcm, itermax_1) {
  ...
  template_fcm_data_processing(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_centers, 2, expected_clusters_length, 1);
}
...
TEST(utest_fcm, itermax_10_simple01) {
  ...
  template_fcm_data_processing(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_centers, 2, expected_clusters_length, 10);
}
