...
TEST(utest_interface_legion, legion_api) {
  std::shared_ptr<pyclustering_package> stimulus =
      pack(legion_stimulus({1, 0, 1, 1, 0, 1, 1, 1, 1, 1}));
  legion_parameters parameters;

  void *legion_network =
      legion_create(10, (unsigned int)connection_t::CONNECTION_ALL_TO_ALL,
                    (void *)&parameters);
  ASSERT_NE(nullptr, legion_network);

  void *dynamic = legion_simulate(legion_network, 10, 10,
                                  (unsigned)solve_type::RUNGE_KUTTA_4, true,
                                  stimulus.get());
