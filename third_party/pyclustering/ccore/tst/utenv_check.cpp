...
void ASSERT_CLUSTER_SIZES(const dataset& p_data,
  ...
  ASSERT_CLUSTER_NOISE_SIZES(p_data, p_actual_clusters,
                             p_expected_cluster_length, {}, -1, p_indexes);
}
...
void ASSERT_CLUSTER_NOISE_SIZES(
  ...
  if (!p_expected_cluster_length.empty()) {
    std::size_t expected_total_size =
        std::accumulate(p_expected_cluster_length.cbegin(),
                        p_expected_cluster_length.cend(), (std::size_t)0);
    ...
    std::vector<size_t> sorted_expected_cluster_length(
