...
static void template_kmeans_length_process_data_common(
    const dataset_ptr& p_data, const dataset& p_start_centers,
    const std::vector<size_t>& p_expected_cluster_length,
    const index_sequence& p_indexes, const bool p_observe,
...
static void template_kmeans_length_process_data_range(
  ...
  template_kmeans_length_process_data_common(
      p_data, p_start_centers, p_expected_cluster_length, p_indexes, false);
}

static void template_kmeans_length_process_data(
    const dataset_ptr& p_data, const dataset& p_start_centers,
    const std::vector<size_t>& p_expected_cluster_length) {
  template_kmeans_length_process_data_range(p_data, p_start_centers,
...
static void template_kmeans_observer(
    ...
    const std::vector<size_t>& p_expected_cluster_length) {
  template_kmeans_length_process_data_common(
      p_data, p_start_centers, p_expected_cluster_length, p_indexes, true);
}

static void template_kmeans_metric(
    const dataset_ptr& p_data, const dataset& p_start_centers,
    const std::vector<size_t>& p_expected_cluster_length,
    const distance_metric<point>& p_metric) {
  template_kmeans_length_process_data_common(
      p_data, p_start_centers, p_expected_cluster_length, {}, false,
      kmeans::DEFAULT_ITERMAX, p_metric);
}

static void template_kmeans_itermax(
    const dataset_ptr& p_data, const dataset& p_start_centers,
    const std::vector<size_t>& p_expected_cluster_length,
    const std::size_t p_itermax) {
  template_kmeans_length_process_data_common(
      p_data, p_start_centers, p_expected_cluster_length, {}, false, p_itermax);
}
...
TEST(utest_kmeans, allocation_sample_simple_01_range) {
  ...
  template_kmeans_length_process_data_range(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_centers, expected_clusters_length, range);
}
...
TEST(utest_kmeans, allocation_sample_simpl_01_euclidean) {
  ...
  template_kmeans_metric(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_centers, expected_clusters_length, metric);
}
...
TEST(utest_kmeans, large_number_centers_sample_simple_01) {
  dataset start_centers = {
      {1.7, 2.6}, {3.7, 4.5}, {4.5, 1.6}, {6.4, 5.0}, {2.2, 2.2}};
...
TEST(utest_kmeans, large_number_centers_sample_simple_02) {
  dataset start_centers = {{-1.5, 0.8},  {-4.9, 5.0}, {2.3, 3.2},
                           {-1.2, -0.8}, {2.5, 2.9},  {6.8, 7.9}};
  std::vector<size_t> expected_clusters_length; /* pass empty */
  template_kmeans_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_02),
      start_centers, expected_clusters_length);
...
TEST(utest_kmeans, large_number_centers_sample_simple_03) {
  dataset start_centers = {{-8.1, 2.3}, {-4.9, 5.5}, {1.3, 8.3}, {-2.6, -1.7},
                           {5.3, 4.2},  {2.1, 0.0},  {1.7, 0.4}};
  std::vector<size_t> expected_clusters_length; /* pass empty */
  template_kmeans_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_03),
      start_centers, expected_clusters_length);
...
TEST(utest_kmeans, collect_evolution_sample_simple_01_range) {
  ...
  template_kmeans_observer(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_centers, range, expected_clusters_length);
}
...
TEST(utest_kmeans, itermax_0) {
  ...
  template_kmeans_itermax(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_centers, expected_clusters_length, 0);
}

TEST(utest_kmeans, itermax_1) {
  dataset start_centers = {{3.7, 5.5}, {6.7, 7.5}};
  std::vector<size_t> expected_clusters_length = {
      5, 5}; /* it is enough to make one step to obtain proper result */
  template_kmeans_itermax(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_centers, expected_clusters_length, 1);
}
...
TEST(utest_kmeans, itermax_10_simple01) {
  ...
  template_kmeans_itermax(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      start_centers, expected_clusters_length, 10);
}
