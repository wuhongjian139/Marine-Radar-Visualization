...
void template_length_process_data(
    const std::shared_ptr<dataset>& data, const dataset& start_centers,
  ...
  if (!expected_cluster_length.empty()) {
    ...
    std::vector<unsigned int> sorted_expected_cluster_length(
...
TEST(utest_xmeans, allocation_bic_sample_simple_01) {
  ...
  template_length_process_data(
      ...
      start_centers, 20, expected_clusters_length,
...
TEST(utest_xmeans, allocation_kmax_less_real_bic_sample_simple_03) {
  ...
  template_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_03),
      start_centers, 3, expected_clusters_length,
      splitting_type::BAYESIAN_INFORMATION_CRITERION);
}
...
TEST(utest_xmeans, allocation_one_cluster_bic_sample_simple_03) {
  ...
  template_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_03),
      start_centers, 1, expected_clusters_length,
      splitting_type::BAYESIAN_INFORMATION_CRITERION);
}
...
TEST(utest_xmeans, allocation_bic_sample_simple_04) {
  dataset start_centers = {
      {1.5, 0.0}, {1.5, 2.0}, {1.5, 4.0}, {1.5, 6.0}, {1.5, 8.0}};
...
TEST(utest_xmeans, allocation_bic_sample_simple_06) {
  ...
  template_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_06),
      start_centers, 20, expected_clusters_length,
      splitting_type::BAYESIAN_INFORMATION_CRITERION);
}
...
TEST(utest_xmeans, same_size_data_and_centers_bic) {
  ...
  template_length_process_data(data, start_centers, 20, {1, 1, 1, 1},
                               splitting_type::BAYESIAN_INFORMATION_CRITERION);
}
...
TEST(utest_xmeans, custom_allocation_bic_01) {
  dataset_ptr data =
      dataset_ptr(new dataset({{0.0, 0.0},
                               {0.1363766466758196, 1.1783713695419944},
                               {3.214688621356828, 1.476853479793315},
                               {2.138463480518565, 4.081574609797002},
                               {0.7396242774728039, 2.87928051726698},
                               {4.392971847058702, 9.373294690055928},
                               {0.9201540078170818, 1.58388176369988},
                               {4.314084965826241, 1.2011928104209428},
                               {11.734694885255871, 7.357322102302481},
                               {3.536693195144401, 7.985381380225232}}));
  dataset start_centers = {{0.5370863032832184, 0.11587434446400902},
                           {0.88032361344512, 0.1499414823211892},
                           {0.32937175324286194, 0.1591103849511586}};

  std::vector<unsigned int> expected_clusters_length = {};

  template_length_process_data(data, start_centers, 5, expected_clusters_length,
                               splitting_type::BAYESIAN_INFORMATION_CRITERION);
}

TEST(utest_xmeans, math_error_domain_bic) {
  dataset_ptr data =
      dataset_ptr(new dataset({{0}, {0}, {10}, {10}, {20}, {20}}));
  dataset start_centers = {{5}, {20}};
  std::vector<unsigned int> expected_clusters_length = {2, 2, 2};
  template_length_process_data(
      data, start_centers, 20, expected_clusters_length,
      splitting_type::BAYESIAN_INFORMATION_CRITERION, 0.0);
}
...
TEST(utest_xmeans, math_error_domain_mndl) {
  ...
  template_length_process_data(
      data, start_centers, 20, expected_clusters_length,
      splitting_type::MINIMUM_NOISELESS_DESCRIPTION_LENGTH, 0.0);
}
...
TEST(performance_xmeans, big_data) {
  ...
  for (std::size_t i = 0; i < repeat; i++) {
    ...
    xmeans solver(centers, 20, 0.0001,
                  splitting_type::BAYESIAN_INFORMATION_CRITERION);
