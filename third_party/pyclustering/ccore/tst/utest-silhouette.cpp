...
void template_correct_scores(const dataset_ptr& p_data,
...
void template_correct_score_data_types(const dataset_ptr& p_data,
                                       const answer& p_answer) {
  silhouette_data result_points, result_matrix;

  dataset matrix;
  distance_matrix(*p_data, distance_metric_factory<point>::euclidean_square(),
                  matrix);

  silhouette().process(*p_data, p_answer.clusters(), silhouette_data_t::POINTS,
                       result_points);
  silhouette().process(matrix, p_answer.clusters(),
                       silhouette_data_t::DISTANCE_MATRIX, result_matrix);
...
TEST(utest_silhouette, correct_score_simple04) {
  template_correct_scores(
      ...
      answer_reader::read(SAMPLE_SIMPLE::SAMPLE_SIMPLE_04));
}
