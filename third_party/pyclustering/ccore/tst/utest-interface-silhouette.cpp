...
TEST(utest_interface_silhouette, silhouette) {
  ...
  dataset matrix;
  distance_matrix(*data, distance_metric_factory<point>::euclidean_square(),
                  matrix);
  ...
  pyclustering_package* result_points = silhouette_algorithm(
      data_package.get(), cluster_package.get(), nullptr, 0);
  pyclustering_package* result_matrix = silhouette_algorithm(
      matrix_package.get(), cluster_package.get(), nullptr, 1);
...
TEST(utest_interface_silhouette, silhouette_ksearch) {
  ...
  pyclustering_package* result =
      silhouette_ksearch_algorithm(sample.get(), 2, 5, 0);
