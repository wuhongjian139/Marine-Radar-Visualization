...
TEST(utest_interface_kmeans, kmeans_api) {
  ...
  pyclustering_package* kmeans_result =
      kmeans_algorithm(sample.get(), centers.get(), 0.001, 200, false, &metric);
  ...
  kmeans_result =
      kmeans_algorithm(sample.get(), centers.get(), 0.1, 100, true, &metric);
  ASSERT_NE(nullptr, kmeans_result);
