...
TEST(utest_hhn, create_10_oscillators) { template_create_delete(10); }

TEST(utest_hhn, create_20_oscillators) { template_create_delete(20); }

TEST(utest_hhn, create_200_oscillators) { template_create_delete(200); }

static void template_collect_dynamic(
    const std::size_t p_num_osc, const std::size_t p_steps,
  ...
  network.simulate(p_steps, 10, solve_type::RUNGE_KUTTA_4, p_stimulus,
                   output_dynamic);

  for (auto& data_type : collected_types) {
    ASSERT_EQ(p_steps + 1,
              output_dynamic.get_peripheral_dynamic()->at(data_type).size());
  }
...
TEST(utest_hhn, collect_membran_size_1_steps_10_unstimulated) {
  ...
  template_collect_dynamic(1, 10, stimulus,
...
TEST(utest_hhn, collect_membran_size_1_steps_50) {
  hhn_stimulus stimulus({1});
  template_collect_dynamic(1, 50, stimulus,
                           {hhn_dynamic::collect::MEMBRANE_POTENTIAL});
}

TEST(utest_hhn, collect_membran_size_1_steps_200) {
  hhn_stimulus stimulus({1});
  template_collect_dynamic(1, 200, stimulus,
                           {hhn_dynamic::collect::MEMBRANE_POTENTIAL});
}

TEST(utest_hhn, collect_membran_size_5_steps_20) {
  hhn_stimulus stimulus({10, 10, 10, 50, 50});
  template_collect_dynamic(5, 20, stimulus,
                           {hhn_dynamic::collect::MEMBRANE_POTENTIAL});
}

TEST(utest_hhn, collect_membran_size_5_steps_50) {
  hhn_stimulus stimulus({50, 50, 0, 10, 10});
  template_collect_dynamic(5, 50, stimulus,
                           {hhn_dynamic::collect::MEMBRANE_POTENTIAL});
}

static void template_ensemble_generation(
    const std::size_t p_num_osc, const std::size_t p_steps,
    const std::size_t p_time, const double p_tolerance,
    const hhn_stimulus& p_stimulus, basic_ensemble_data& p_expected_ensembles,
  ...
  for (std::size_t i = 0; (i < attempts) && (result != true); i++) {
    ...
    /* simulate and check collected outputs */
    network.simulate(p_steps, (double)p_time, solve_type::RUNGE_KUTTA_4,
                     p_stimulus, output_dynamic);

    basic_ensemble_data ensembles;
    basic_ensemble dead_neurons;

    hhn_dynamic::evolution_dynamic& membrane_dynamic =
        output_dynamic.get_peripheral_dynamic(
            hhn_dynamic::collect::MEMBRANE_POTENTIAL);
    dynamic_analyser(p_tolerance)
        .allocate_sync_ensembles(membrane_dynamic, ensembles, dead_neurons);

    if (!COMPARE_SYNC_ENSEMBLES(ensembles, p_expected_ensembles, dead_neurons,
                                p_expected_dead_neurons)) {
      continue;
    }
...
TEST(utest_hhn, one_without_stimulation) {
  ...
  template_ensemble_generation(1, 400, 100, 0.0, {0}, expected_ensembles,
                               dead_neurons);
}
...
TEST(utest_hhn, one_with_stimulation) {
  ...
  template_ensemble_generation(1, 400, 100, 0.0, {25}, expected_ensembles,
                               dead_neurons);
}
...
TEST(utest_hhn, global_sync) {
  ...
  template_ensemble_generation(3, 300, 50, 0.0, {27, 27, 27},
                               expected_ensembles, dead_neurons);
}
...
TEST(utest_hhn, without_stimulation) {
  ...
  template_ensemble_generation(3, 400, 100, 0.0, {0, 0, 0}, expected_ensembles,
                               dead_neurons);
}
...
TEST(utest_hhn, one_with_one_without_stimulation) {
  ...
  template_ensemble_generation(2, 400, 100, 0.0, {0, 25}, expected_ensembles,
                               dead_neurons);
}
...
TEST(utest_hhn, two_sync_ensembles_01) {
  ...
  template_ensemble_generation(4, 800, 200, 0.1, {20, 20, 80, 80},
                               expected_ensembles, dead_neurons);
}
...
TEST(utest_hhn, two_sync_ensembles_02) {
  ...
  template_ensemble_generation(6, 800, 200, 0.1, {20, 20, 20, 50, 50, 50},
                               expected_ensembles, dead_neurons);
}

#endif

static void template_write_read_dynamic(
    const std::size_t p_num_osc, const std::size_t p_steps,
    const std::size_t p_time, const hhn_stimulus& p_stimulus,
...
TEST(utest_hhn, wr_ten_oscillators) {
  std::vector<hhn_dynamic::collect> enables = {
      hhn_dynamic::collect::MEMBRANE_POTENTIAL};
  template_write_read_dynamic(
      10, 100, 10, {10, 10, 10, 12, 12, 12, 20, 20, 20, 20}, enables);
}
