...
TEST(utest_interface_kmedoids, kmedoids_api) {
  ...
  pyclustering_package* kmedoids_result =
      kmedoids_algorithm(sample.get(), medoids.get(), 0.001, 100, &metric, 0);
...
TEST(utest_interface_kmedoids, kmedoids_api_null_metric) {
  ...
  pyclustering_package* kmedoids_result =
      kmedoids_algorithm(sample.get(), medoids.get(), 0.001, 100, nullptr, 0);
  ASSERT_NE(nullptr, kmedoids_result);
