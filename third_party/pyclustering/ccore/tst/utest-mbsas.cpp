...
static void template_mbsas_length_process_data(
  ...
  for (auto& repr : actual_repr) ASSERT_EQ(data[0].size(), repr.size());
...
TEST(utest_mbsas, allocation_one_dimension_points_1) {
  ...
  template_mbsas_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_07), 3,
      1.0, expected_clusters_length);
}
...
TEST(utest_mbsas, allocation_one_dimension_points_2) {
  ...
  template_mbsas_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_09), 3,
      1.0, expected_clusters_length);
}
