...
class utest_kdtree : public ::testing::Test {
 ...
 protected:
  ...
  virtual void TemplateTestInsertSearchRemove(
      const dataset &p_data, const double radius_search = 10.0) {
    tree = kdtree();
    ...
    for (std::size_t index = 0; index < p_data.size(); index++) {
      ...
      kdtree_searcher::rule_store rule = [&index, &index_points](
      ...
      ASSERT_TRUE(std::find(index_points.begin(), index_points.end(), index) ==
                  index_points.end());
    }
  ...
  virtual void TemplateInsertFindRemoveByCoordinatesAndPayload(
      const dataset &p_data, const std::vector<std::size_t> &p_payloads) {
    tree = kdtree();
    ...
    for (auto &node : nodes) {
      kdnode::ptr found_node =
          tree.find_node(node->get_data(), node->get_payload());
    ...
    for (std::size_t i = 0; i < nodes.size(); i++) {
      ...
      kdnode::ptr found_node =
          tree.find_node(nodes[i]->get_data(), nodes[i]->get_payload());
      ASSERT_EQ(nullptr, found_node);

      for (std::size_t j = i + 1; j < nodes.size(); j++) {
        found_node =
            tree.find_node(nodes[j]->get_data(), nodes[j]->get_payload());
...
TEST_F(utest_kdtree, insert_remove_node) {
  ...
  for (std::size_t index = 0; index < test_sample_point_vector.size();
       index++) {
  ...
  for (std::size_t index = 0; index < test_sample_point_vector.size();
    ...
    for (std::size_t next_index = index + 1;
         next_index < test_sample_point_vector.size(); next_index++) {
...
TEST_F(utest_kdtree, find_without_insertion) {
  ...
  for (unsigned int index = 0; index < test_sample_point_vector.size();
       index++) {
    std::vector<double> &point = test_sample_point_vector[index];
...
TEST_F(utest_kdtree, insert_search_remove_simple_08) {
  ...
  const double search_radius =
      farthest_distance(*sample, distance_metric_factory<point>::euclidean());
...
TEST_F(utest_kdtree, insert_search_remove_with_payload_identical_data_5) {
  dataset data = {{75, 75}, {75, 75}, {75, 75}, {75, 75},
