...
static std::shared_ptr<optics_data> template_optics_length_process_data(
    const std::shared_ptr<dataset>& p_data, const double p_radius,
    const size_t p_neighbors, const size_t p_amount_clusters,
    const std::vector<size_t>& p_expected_cluster_length) {
  std::shared_ptr<optics_data> ptr_output_result =
      std::make_shared<optics_data>();
  ...
  if (p_amount_clusters > 0) {
    EXPECT_EQ(p_expected_cluster_length.size(),
              ordering_analyser::extract_cluster_amount(
                  ptr_output_result->cluster_ordering(),
                  ptr_output_result->get_radius()));
  }

  EXPECT_EQ(p_data->size(), objects.size());
  for (const auto& object : objects) {
    EXPECT_TRUE(object.m_core_distance == optics::NONE_DISTANCE ||
                object.m_core_distance >= 0.0);
    EXPECT_TRUE(object.m_reachability_distance == optics::NONE_DISTANCE ||
                object.m_reachability_distance >= 0.0);
...
template_optics_length_process_distance_matrix(
    const std::shared_ptr<dataset>& p_data, const double p_radius,
    const size_t p_neighbors, const size_t p_amount_clusters,
    const std::vector<size_t>& p_expected_cluster_length) {
  std::shared_ptr<optics_data> ptr_output_result =
...
TEST(utest_optics, allocation_sample_simple_01) {
  ...
  template_optics_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      0.4, 2, 0, expected_clusters_length);
}
...
TEST(utest_optics, allocation_one_allocation_simple_01) {
  ...
  template_optics_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      10.0, 1, 0, expected_clusters_length);
  template_optics_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      9.0, 1, 0, expected_clusters_length);
  template_optics_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      5.0, 1, 0, expected_clusters_length);
}
...
TEST(utest_optics, allocation_sample_simple_02) {
  ...
  template_optics_length_process_data(
      ...
      1.0, 2, 0, expected_clusters_length);
}
...
TEST(utest_optics, allocation_sample_simple_03) {
  ...
  template_optics_length_process_data(
      ...
      0.7, 3, 0, expected_clusters_length);
}
...
TEST(utest_optics, allocation_one_dimension) {
  ...
  template_optics_length_process_data(
      ...
      3.0, 3, 0, expected_clusters_length);
}
...
TEST(utest_optics, allocation_sample_simple_02_large_radius) {
  ...
  template_optics_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_02),
      5.0, 2, 3, expected_clusters_length);
}
...
TEST(utest_optics, allocation_sample_simple_03_large_radius) {
  ...
  template_optics_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_03),
      7.0, 4, 4, expected_clusters_length);
}
...
TEST(utest_optics, allocation_sample_simple_04_large_radius) {
  ...
  template_optics_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_04),
      50.0, 5, 5, expected_clusters_length);
}
...
TEST(utest_optics, allocation_sample_simple_05_large_radius) {
  ...
  template_optics_length_process_data(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_05),
      10.0, 10, 4, expected_clusters_length);
}
...
TEST(utest_optics, allocation_sample_lsun_large_radius_10) {
  ...
  template_optics_length_process_data(
      fcps_sample_factory::create_sample(FCPS_SAMPLE::LSUN), 1.0, 3, 3,
      expected_clusters_length);
}
...
TEST(utest_optics, allocation_sample_lsun_large_radius_19) {
  const std::vector<size_t> expected_clusters_length = {99, 100, 202};
  template_optics_length_process_data(
      fcps_sample_factory::create_sample(FCPS_SAMPLE::LSUN), 1.9, 3, 3,
      expected_clusters_length);
}

#endif

static std::shared_ptr<optics_data> template_optics_noise_allocation(
    const std::shared_ptr<dataset>& p_data, const double p_radius,
    const size_t p_neighbors, const size_t p_amount_clusters,
    const std::vector<size_t>& p_expected_cluster_length,
    const std::size_t p_noise_length) {
  std::shared_ptr<optics_data> ptr_output_result =
      template_optics_length_process_data(p_data, p_radius, p_neighbors,
                                          p_amount_clusters,
...
static std::shared_ptr<optics_data>
template_optics_noise_allocation_distance_matrix(
    const std::shared_ptr<dataset>& p_data, const double p_radius,
    const size_t p_neighbors, const size_t p_amount_clusters,
    const std::vector<size_t>& p_expected_cluster_length,
    const std::size_t p_noise_length) {
  std::shared_ptr<optics_data> ptr_output_result =
      template_optics_length_process_distance_matrix(
          p_data, p_radius, p_neighbors, p_amount_clusters,
          p_expected_cluster_length);
  EXPECT_EQ(p_noise_length, ptr_output_result->noise().size());
...
TEST(utest_optics, noise_allocation_sample_simple_01) {
  ...
  template_optics_noise_allocation(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_01),
      10.0, 20, 0, expected_clusters_length, 10);
}
...
TEST(utest_optics, noise_allocation_sample_simple_02) {
  ...
  template_optics_noise_allocation(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_02),
      0.5, 20, 0, expected_clusters_length, 23);
}
...
TEST(utest_optics, noise_cluster_allocation_sample_simple_02) {
  ...
  template_optics_noise_allocation(
      simple_sample_factory::create_sample(SAMPLE_SIMPLE::SAMPLE_SIMPLE_02),
      2.0, 9, 0, expected_clusters_length, 13);
}
...
TEST(performance_optics, big_data) {
  ...
  std::cout << "Clustering time: '" << difference.count() / repeat << "' sec."
            << std::endl;
}
