...
/**
 *
 * @brief    Checks that clusters have all allocated objects together from input
 * data and that sizes of allocated clusters are expected.
 * @details  If 'p_expected_cluster_length' is empty than only total number of
 * allocated object is checked - clusters consist of each point index from input
 * data.
 *
 * @param[in] p_data: data that has been processed.
 * @param[in] p_actual_clusters: allocated clusters.
 * @param[in] p_expected_cluster_length: expected clusters length - can be
 * empty.
...
void ASSERT_CLUSTER_NOISE_SIZES(
    const dataset& p_data, const cluster_sequence& p_actual_clusters,
    const std::vector<std::size_t>& p_expected_cluster_length,
    const noise& p_actual_noise, const std::size_t& p_expected_noise_length,
    const index_sequence& p_indexes = {});
...
template <typename EnsemblesType>
void ASSERT_SYNC_ENSEMBLES(
    EnsemblesType& p_ensembles, EnsemblesType& p_expected_ensembles,
    typename EnsemblesType::value_type& p_dead,
    typename EnsemblesType::value_type& p_expected_dead) {
  ASSERT_TRUE(COMPARE_SYNC_ENSEMBLES(p_ensembles, p_expected_ensembles, p_dead,
                                     p_expected_dead));
}