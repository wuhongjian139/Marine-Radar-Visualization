...
thread_pool::thread_pool() { initialize(DEFAULT_POOL_SIZE); }

thread_pool::thread_pool(const std::size_t p_size) { initialize(p_size); }
...
std::size_t thread_pool::size() const { return m_pool.size(); }
...
void thread_pool::initialize(const std::size_t p_size) {
  ...
  thread_executor::task_getter getter =
      std::bind(&thread_pool::get_task, this, std::placeholders::_1);
...
void thread_pool::get_task(task::ptr& p_task) {
  ...
  while (m_queue.empty() && !m_stop) {
    m_queue_not_empty_cond.wait(lock_common,
