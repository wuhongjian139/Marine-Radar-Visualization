...
void *sync_create_network(const unsigned int size, const double weight_factor,
                          const double frequency_factor,
                          const unsigned int connection_type,
                          const unsigned int initial_phases) {
  return new sync_network(size, weight_factor, frequency_factor,
                          (connection_t)connection_type,
                          (initial_type)initial_phases);
}
...
void *sync_simulate_static(const void *pointer_network, unsigned int steps,
                           const double time, const unsigned int solver,
                           const bool collect_dynamic) {
  sync_network *network = (sync_network *)pointer_network;

  sync_dynamic *dynamic = new sync_dynamic();
  network->simulate_static(steps, time, (solve_type)solver, collect_dynamic,
                           (*dynamic));

  return (void *)dynamic;
}

void *sync_simulate_dynamic(const void *pointer_network, const double order,
                            const unsigned int solver,
                            const bool collect_dynamic, const double step,
                            const double step_int,
                            const double threshold_changes) {
  sync_network *network = (sync_network *)pointer_network;

  sync_dynamic *dynamic = new sync_dynamic();
  network->simulate_dynamic(order, step, (solve_type)solver, collect_dynamic,
                            (*dynamic));
...
pyclustering_package *sync_connectivity_matrix(const void *pointer_network) {
  std::shared_ptr<adjacency_collection> connections =
      ((sync_network *)pointer_network)->connections();

  pyclustering_package *package =
      new pyclustering_package(pyclustering_data_t::PYCLUSTERING_TYPE_LIST);
  ...
  for (std::size_t i = 0; i < package->size; i++) {
    pyclustering_package *subpackage =
        new pyclustering_package(pyclustering_data_t::PYCLUSTERING_TYPE_DOUBLE);
    ...
    for (std::size_t j = 0; j < subpackage->size; j++) {
      ((double *)subpackage->data)[j] =
          connections->has_connection(i, j) ? 1.0 : 0.0;
    }
...
pyclustering_package *sync_dynamic_allocate_sync_ensembles(
    const void *pointer_dynamic, const double tolerance,
    const std::size_t iteration) {
  ensemble_data<sync_ensemble> ensembles;

  ((sync_dynamic *)pointer_dynamic)
      ->allocate_sync_ensembles(tolerance, iteration, ensembles);

  pyclustering_package *package = create_package(&ensembles);
  return package;
}

pyclustering_package *sync_dynamic_allocate_correlation_matrix(
    const void *pointer_dynamic, const std::size_t iteration) {
  sync_corr_matrix matrix;
  ((sync_dynamic *)pointer_dynamic)
      ->allocate_correlation_matrix(iteration, matrix);
...
pyclustering_package *sync_dynamic_get_output(const void *pointer_dynamic) {
  ...
  for (std::size_t i = 0; i < package->size; i++) {
    ((pyclustering_package **)package->data)[i] =
        create_package(&dynamic[i].m_phase);
  }

  return package;
}

pyclustering_package *sync_dynamic_calculate_order(const void *p_pointer,
...
pyclustering_package *sync_dynamic_calculate_local_order(
  ...
  dynamic.calculate_local_order_parameter(network.connections(), p_start,
                                          p_stop, local_order_evolution);
