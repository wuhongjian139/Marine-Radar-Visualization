...
void *som_create(const size_t num_rows, const size_t num_cols,
                 const size_t type_conn, const void *parameters) {
  return new som(num_rows, num_cols, (som_conn_type)type_conn,
                 *((som_parameters *)parameters));
}

void som_destroy(const void *pointer) { delete (som *)pointer; }

void som_load(const void *p_pointer, const pyclustering_package *p_weights,
...
size_t som_train(const void *pointer, const pyclustering_package *const sample,
...
size_t som_simulate(const void *pointer,
                    const pyclustering_package *const p_pattern) {
...
pyclustering_package *som_get_capture_objects(const void *pointer) {
  const som_gain_sequence &capture_objects =
      ((som *)pointer)->get_capture_objects();
