...
void *syncpr_create(const unsigned int num_osc, const double increase_strength1,
...
void syncpr_train(const void *pointer_network, const void *const patterns) {
  ...
  const pyclustering_package *const package_patterns =
      static_cast<const pyclustering_package *const>(patterns);
...
void *syncpr_simulate_static(const void *pointer_network, unsigned int steps,
                             const double time, const void *const pattern,
                             const unsigned int solver,
                             const bool collect_dynamic) {
  syncpr *network = (syncpr *)pointer_network;

  const pyclustering_package *const package_pattern =
      static_cast<const pyclustering_package *const>(pattern);
  syncpr_pattern external_pattern(
      static_cast<int *>(package_pattern->data),
      static_cast<int *>(package_pattern->data) + package_pattern->size);

  syncpr_dynamic *dynamic = new syncpr_dynamic();
  network->simulate_static(steps, time, external_pattern, (solve_type)solver,
...
void *syncpr_simulate_dynamic(const void *pointer_network,
                              const void *const pattern, const double order,
                              const unsigned int solver,
  ...
  const pyclustering_package *const package_pattern =
      static_cast<const pyclustering_package *const>(pattern);
  syncpr_pattern external_pattern(
      (int *)package_pattern->data,
      ((int *)package_pattern->data) + package_pattern->size);

  syncpr_dynamic *dynamic = new syncpr_dynamic();
  network->simulate_dynamic(external_pattern, order, step, (solve_type)solver,
                            collect_dynamic, (*dynamic));

  return (void *)dynamic;
}

double syncpr_memory_order(const void *pointer_network,
...
pyclustering_package *syncpr_dynamic_allocate_sync_ensembles(
    const void *pointer_dynamic, const double tolerance) {
  return sync_dynamic_allocate_sync_ensembles(
      pointer_dynamic, tolerance, syncpr_dynamic_get_size(pointer_dynamic) - 1);
}
