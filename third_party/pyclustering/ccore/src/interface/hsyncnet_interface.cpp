...
void *hsyncnet_create_network(const pyclustering_package *const p_sample,
  ...
  return new hsyncnet(&input_data, p_number_clusters,
                      (initial_type)p_initial_phases, p_initial_neighbors,
                      p_increase_persent);
}
...
void *hsyncnet_process(const void *p_pointer_network, const double p_order,
