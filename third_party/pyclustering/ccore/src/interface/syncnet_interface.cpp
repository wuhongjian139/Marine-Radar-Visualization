...
void *syncnet_create_network(const pyclustering_package *const p_sample,
  ...
  return new syncnet(&input_data, p_connectivity_radius, p_enable_conn_weight,
                     (initial_type)p_initial_phases);
}
...
void *syncnet_process(const void *p_pointer_network, const double p_order,
  ...
  network->process(p_order, (solve_type)p_solver, p_collect_dynamic,
                   (*analyser));
