...
void *hhn_dynamic_create(bool p_collect_membrane,
...
void hhn_simulate(const void *p_network_pointer, const std::size_t p_steps,
  ...
  hhn_stimulus stimulus_vector((double *)p_stimulus->data,
                               ((double *)p_stimulus->data) + p_stimulus->size);

  network->simulate(p_steps, p_time, (solve_type)p_solver, stimulus_vector,
                    *dynamic);
}

pyclustering_package *hhn_dynamic_get_peripheral_evolution(
    const void *p_output_dynamic, const std::size_t p_collection_index) {
  hhn_dynamic *dynamic = (hhn_dynamic *)p_output_dynamic;
  hhn_dynamic::evolution_dynamic &evolution =
      dynamic->get_peripheral_dynamic((hhn_dynamic::collect)p_collection_index);

  pyclustering_package *package = create_package(&evolution);
  return package;
}

pyclustering_package *hhn_dynamic_get_central_evolution(
    const void *p_output_dynamic, const std::size_t p_collection_index) {
  hhn_dynamic *dynamic = (hhn_dynamic *)p_output_dynamic;
  hhn_dynamic::evolution_dynamic &evolution =
      dynamic->get_central_dynamic((hhn_dynamic::collect)p_collection_index);
