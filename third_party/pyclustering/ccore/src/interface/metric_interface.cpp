...
void *metric_create(const std::size_t p_type,
  ...
  switch (p_type) {
    ...
    case MINKOWSKI: {
      ...
      distance_metric<point> metric =
          distance_metric_factory<point>::minkowski(arguments[0]);
      return new distance_metric<point>(std::move(metric));
    }

    case CANBERRA: {
      distance_metric<point> metric =
          distance_metric_factory<point>::canberra();
      return new distance_metric<point>(std::move(metric));
    }

    case CHI_SQUARE: {
      distance_metric<point> metric =
          distance_metric_factory<point>::chi_square();
      return new distance_metric<point>(std::move(metric));
    }
    ...
    case GOWER: {
      ...
      distance_metric<point> metric =
          distance_metric_factory<point>::gower(max_range);
      return new distance_metric<point>(std::move(metric));
    }
    ...
    case USER_DEFINED: {
      ...
      distance_metric<point> metric =
          distance_metric_factory<point>::user_defined(functor_wrapper);
      return new distance_metric<point>(std::move(metric));
    }
...
double metric_calculate(const void *p_pointer_metric,
  ...
  distance_metric<point> &metric =
      *((distance_metric<point> *)p_pointer_metric);
