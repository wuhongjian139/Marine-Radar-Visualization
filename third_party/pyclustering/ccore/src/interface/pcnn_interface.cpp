...
void *pcnn_create(const unsigned int p_size,
                  const unsigned int p_connection_type,
                  const unsigned int p_height, const unsigned int p_width,
                  const void *const p_parameters) {
  pcnn *pcnn_network =
      new pcnn(p_size, (connection_t)p_connection_type, p_height, p_width,
               *((pcnn_parameters *)p_parameters));
  return (void *)pcnn_network;
}

void pcnn_destroy(const void *p_pointer) { delete (pcnn *)p_pointer; }

void *pcnn_simulate(const void *p_pointer, const unsigned int p_steps,
                    const void *const p_stimulus) {
  const pyclustering_package *const package_stimulus =
      static_cast<const pyclustering_package *const>(p_stimulus);
  pcnn_stimulus stimulus_vector(
      (double *)package_stimulus->data,
      ((double *)package_stimulus->data) + package_stimulus->size);
...
pyclustering_package *pcnn_dynamic_allocate_sync_ensembles(
    const void *pointer) {
...
pyclustering_package *pcnn_dynamic_allocate_spike_ensembles(
    const void *pointer) {
...
pyclustering_package *pcnn_dynamic_get_output(const void *pointer) {
  ...
  for (std::size_t i = 0; i < package->size; i++) {
    ((pyclustering_package **)package->data)[i] =
        create_package(&dynamic[i].m_output);
  }
