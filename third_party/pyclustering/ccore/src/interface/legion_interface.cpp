...
void *legion_create(const unsigned int p_size,
                    const unsigned int p_connection_type,
                    const void *const p_parameters) {
  legion_network *network =
      new legion_network(p_size, (connection_t)p_connection_type,
                         *((legion_parameters *)p_parameters));
  return (void *)network;
}
...
void *legion_simulate(const void *p_network_pointer, const unsigned int p_steps,
                      const double p_time, const unsigned int p_solver,
                      const bool p_collect_dynamic,
                      const pyclustering_package *const p_stimulus) {
  legion_stimulus stimulus_vector(
      (double *)p_stimulus->data,
      ((double *)p_stimulus->data) + p_stimulus->size);

  legion_dynamic *dynamic = new legion_dynamic();
  ((legion_network *)p_network_pointer)
      ->simulate(p_steps, p_time, (solve_type)p_solver, p_collect_dynamic,
                 stimulus_vector, (*dynamic));
...
pyclustering_package *legion_dynamic_get_inhibitory_output(
