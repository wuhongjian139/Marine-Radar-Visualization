...
cure_cluster::cure_cluster()
    : mean(nullptr), closest(nullptr), distance_closest(0) {
...
cure_cluster::cure_cluster(std::vector<double> *point)
    : closest(nullptr), distance_closest(0) {
  mean = new std::vector<double>(*point);

  points = new std::vector<std::vector<double> *>(
      1, point); /* use user data points */
  rep = new std::vector<std::vector<double> *>(
      1, new std::vector<double>(
             *point)); /* it's our - despite at the beginning it the same */
}
...
void cure_cluster::insert_points(
    std::vector<std::vector<double> *> *append_points) {
...
bool cure_cluster_comparator::operator()(const cure_cluster *const obj1,
                                         const cure_cluster *const obj2) const {
...
void cure_queue::create_queue(const dataset *data) {
  ...
  for (auto &data_point : (*data)) {
    cure_cluster *cluster =
        new cure_cluster((std::vector<double> *)&data_point);
...
double cure_queue::get_distance(cure_cluster *cluster1,
                                cure_cluster *cluster2) {
...
void cure_queue::merge(cure_cluster *cluster1, cure_cluster *cluster2,
                       const size_t number_repr_points,
  ...
  merged_cluster->mean =
      new std::vector<double>((*cluster1->points)[0]->size(), 0);

  // If all elements were same then avoid calculating the mean mathematically as
  // it may lead to precision error.
  if (are_all_elements_same(merged_cluster)) {
    ...
    for (std::size_t dimension = 0; dimension < merged_cluster->mean->size();
         dimension++) {
      (*merged_cluster->mean)[dimension] =
          (cluster1->points->size() * (*cluster1->mean)[dimension] +
           cluster2->points->size() * (*cluster2->mean)[dimension]) /
          (cluster1->points->size() + cluster2->points->size());
    }
  }

  std::set<std::vector<double> *> *temporary =
      new std::set<std::vector<double> *>();
  ...
  for (std::size_t index = 0; index < number_repr_points; index++) {
    ...
    for (auto &point : *(merged_cluster->points)) {
      ...
      if (index == 0) {
        minimal_distance =
            euclidean_distance_square(*point, *(merged_cluster->mean));
      } else {
  ...
  for (auto &point : *temporary) {
    std::vector<double> *representative_point =
        new std::vector<double>(point->size(), 0);
    for (std::size_t index = 0; index < point->size(); index++) {
      (*representative_point)[index] =
          (*point)[index] +
  ...
  if (!queue->empty()) {
    ...
    merged_cluster->distance_closest =
        get_distance(merged_cluster, merged_cluster->closest);

    /* relocation request */
    for (auto iterator_cluster = queue->begin();
         iterator_cluster != queue->end(); iterator_cluster++) {
      ...
      if ((cluster->closest == cluster1) || (cluster->closest == cluster2)) {
        /* If previous distance was less then distance to new cluster then
         * nearest cluster should be found in the tree. */
        if (cluster->distance_closest < distance) {
          cure_cluster *nearest_cluster = nullptr;
          double nearest_distance = std::numeric_limits<double>::max();

          for (auto &point : *(cluster->rep)) {
            /* we are using Eucliean Square metric, but kdtree searcher requires
             * common Eucliean distance (but output results are square) */
            kdtree_searcher searcher(*point, tree->get_root(),
            ...
            for (std::size_t index = 0; index < nearest_nodes.size(); index++) {
              if ((nearest_node_distances[index] < nearest_distance) &&
                  (nearest_nodes[index]->get_payload() != cluster)) {
                nearest_distance = nearest_node_distances[index];
                nearest_cluster = static_cast<cure_cluster *>(
                    nearest_nodes[index]->get_payload());
              }
            }
          }

          if (nearest_cluster == nullptr) {
            relocation_request.emplace_back(iterator_cluster, merged_cluster,
                                            distance);
          } else {
            relocation_request.emplace_back(iterator_cluster, nearest_cluster,
                                            nearest_distance);
          }
        } else {
          relocation_request.emplace_back(iterator_cluster, merged_cluster,
...
void cure_queue::remove_cluster(cure_cluster *removed_cluster) {
  ...
  throw std::runtime_error(
      "CURE queue corruption detected, impossible to remove cluster from the "
      "queue. Please report to 'pyclustering@yandex.ru'.");
}
...
relocation_info::relocation_info(const cure_queue::iterator &cluster_iterator,
                                 cure_cluster *closest_cluster,
...
cure::cure(const size_t clusters_number, size_t points_number,
           const double level_compression)
    : queue(nullptr),
...
void cure::process(const dataset &p_data, cluster_data &p_result) {
  ...
  for (auto cure_cluster = queue->begin(); cure_cluster != queue->end();
       ++cure_cluster, cluster_index++) {
    cluster &standard_cluster = clusters[cluster_index];
    for (auto point = (*cure_cluster)->points->begin();
         point != (*cure_cluster)->points->end(); ++point) {
