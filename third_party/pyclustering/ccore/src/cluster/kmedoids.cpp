...
const std::size_t kmedoids::OBJECT_ALREADY_CONTAINED =
...
kmedoids::kmedoids(const medoid_sequence &p_initial_medoids,
                   const double p_tolerance, const std::size_t p_itermax,
...
void kmedoids::process(const dataset &p_data, const kmedoids_data_t p_type,
  ...
  for (std::size_t iteration = 0;
       (iteration < m_itermax) && (changes > m_tolerance); iteration++) {
    update_clusters();
...
void kmedoids::update_clusters() {
  ...
  parallel_for(std::size_t(0), m_data_ptr->size(),
               [this, &medoids, &cluster_markers](const std::size_t p_index) {
                 cluster_markers[p_index] =
                     find_appropriate_cluster(p_index, medoids);
               });

  for (std::size_t index_point = 0; index_point < m_data_ptr->size();
       index_point++) {
...
void kmedoids::calculate_medoids(cluster &p_medoids) {
  ...
  parallel_for(std::size_t(0), clusters.size(),
               [this, &p_medoids, &clusters](const std::size_t index) {
...
kmedoids::distance_calculator kmedoids::create_distance_calculator(
    const kmedoids_data_t p_type) {
...
std::size_t kmedoids::find_appropriate_cluster(const std::size_t p_index,
                                               medoid_sequence &p_medoids) {
  if (std::find(p_medoids.begin(), p_medoids.end(), p_index) !=
      p_medoids.cend()) {
