...
fcm::fcm(const dataset &p_initial_centers, const double p_m,
...
void fcm::process(const dataset &p_data, cluster_data &p_result) {
  ...
  m_ptr_result->membership().resize(m_ptr_data->size(),
                                    point(m_initial_centers.size(), 0.0));
...
double fcm::update_centers() {
  ...
  parallel_for(std::size_t(0), amount_centers,
               [this, &changes](const std::size_t p_index) {
...
double fcm::update_center(const std::size_t p_index) {
  ...
  for (std::size_t j = 0; j < data_length; j++) {
    for (std::size_t dimension = 0; dimension < dimensions; dimension++) {
      dividend[dimension] += m_ptr_data->at(j).at(dimension) *
                             m_ptr_result->membership()[j][p_index];
  ...
  double change =
      euclidean_distance(update_center, m_ptr_result->centers().at(p_index));
...
void fcm::update_point_membership(const std::size_t p_index) {
  ...
  for (std::size_t j = 0; j < center_amount; j++) {
    differences[j] = euclidean_distance_square(m_ptr_data->at(p_index),
                                               m_ptr_result->centers().at(j));
  }
