...
const std::size_t kmeans_plus_plus::FARTHEST_CENTER_CANDIDATE =
    std::numeric_limits<std::size_t>::max();
const std::size_t kmeans_plus_plus::INVALID_INDEX =
    std::numeric_limits<std::size_t>::max();

kmeans_plus_plus::kmeans_plus_plus(const std::size_t p_amount,
                                   const std::size_t p_candidates) noexcept
    : m_amount(p_amount),
...
kmeans_plus_plus::kmeans_plus_plus(const std::size_t p_amount,
    ...
    : m_amount(p_amount), m_candidates(p_candidates), m_dist_func(p_metric) {}
...
void kmeans_plus_plus::store_center(const store_result &p_proc,
...
void kmeans_plus_plus::store_temporal_params(
    const dataset &p_data, const index_sequence &p_indexes) const {
  if (p_data.empty()) {
  ...
  if (p_data.size() < m_amount) {
    throw std::invalid_argument(
        "Amount of objects should be equal or greater then amount of "
        "initialized centers.");
  }

  if (!p_indexes.empty() && p_indexes.size() < m_amount) {
    throw std::invalid_argument(
        "Amount of objects defined by range should be equal or greater then "
        "amount of initialized centers.");
  }
...
kmeans_plus_plus::center_description kmeans_plus_plus::get_first_center()
    const {
  std::size_t length =
      m_indexes_ptr->empty() ? m_data_ptr->size() : m_indexes_ptr->size();
  ...
  const auto &center = m_indexes_ptr->empty()
                           ? (*m_data_ptr)[index]
                           : (*m_data_ptr)[(*m_indexes_ptr)[index]];
...
void kmeans_plus_plus::calculate_shortest_distances(
...
double kmeans_plus_plus::get_shortest_distance(const point &p_point) const {
  ...
  for (auto &index_point : m_allocated_indexes) {
    double distance =
        std::abs(m_dist_func(p_point, m_data_ptr->at(index_point)));
...
void kmeans_plus_plus::calculate_probabilities(
...
std::size_t kmeans_plus_plus::get_probable_center(
    const std::vector<double> &p_distances,
  ...
  for (std::size_t i = 0; i < m_candidates; i++) {
    ...
    } else if (p_distances[current_index_candidate] >
               p_distances[best_index_candidate]) {
