...
void syncnet_analyser::allocate_clusters(const double eps,
                                         syncnet_cluster_data &data) {
  allocate_sync_ensembles(eps, data);
}

syncnet::syncnet(std::vector<std::vector<double> > *input_data,
                 const double connectivity_radius,
                 const bool enable_conn_weight,
                 const initial_type initial_phases)
    : sync_network(input_data->size(), 1, 0, connection_t::CONNECTION_NONE,
                   initial_type::RANDOM_GAUSSIAN) {
  equation<double> oscillator_equation =
      std::bind(&syncnet::phase_kuramoto_equation, this, _1, _2, _3, _4);
...
void syncnet::create_connections(const double connectivity_radius,
  ...
  if (enable_conn_weight == true) {
    ...
    distance_conn_weights =
        new std::vector<std::vector<double> >(size(), instance);
  } else {
    distance_conn_weights = nullptr;
  }
  ...
  for (std::size_t i = 0; i < size(); i++) {
    for (std::size_t j = i + 1; j < size(); j++) {
      double distance = euclidean_distance_square((*oscillator_locations)[i],
                                                  (*oscillator_locations)[j]);
  ...
  if (enable_conn_weight == true) {
    ...
    for (std::size_t i = 0; i < size(); i++) {
      for (std::size_t j = i + 1; j < size(); j++) {
        double value_weight =
            ((*distance_conn_weights)[i][j] - subtractor) / multiplier;
...
double syncnet::phase_kuramoto(const double t, const double teta,
  ...
  if (distance_conn_weights != nullptr) {
    for (std::size_t k = 0; k < size(); k++) {
      if (m_connections->has_connection(index, k)) {
        phase += (*distance_conn_weights)[index][k] *
                 std::sin(m_oscillators[k].phase - teta);
        num_neighbors++;
      }
    }
  } else {
...
void syncnet::phase_kuramoto_equation(const double t,
...
void syncnet::process(const double order, const solve_type solver,
