...
gmeans::gmeans(const std::size_t p_k_initial, const double p_tolerance,
...
void gmeans::process(const dataset &p_data, cluster_data &p_result) {
  ...
  if (!m_ptr_result) {
    throw std::invalid_argument(
        "Invalid result storage is specified: impossible to cast to "
        "'gmeans_data'.");
  }

  search_optimal_parameters(p_data, m_amount, m_ptr_result->clusters(),
                            m_ptr_result->centers());
...
void gmeans::search_optimal_parameters(const dataset &p_data,
  ...
  for (std::size_t i = 0; i < m_repeat; i++) {
    ...
    kmeans_plus_plus(p_amount, get_amount_candidates(p_data))
        .initialize(p_data, initial_centers);
...
void gmeans::split_and_search_optimal(const cluster &p_cluster,
                                      dataset &p_centers) const {
  if (p_cluster.size() == 1) {
    return;
  }
...
bool gmeans::is_null_hypothesis(const dataset &p_data, const point &p_center1,
  ...
  return (estimation < critical.back()); /* true - Gaussian distribution, false
                                            - not a Gaussian distribution */
}

std::size_t gmeans::get_amount_candidates(const dataset &p_data) {
  return (p_data.size() > DEFAULT_CANDIDATES) ? DEFAULT_CANDIDATES
                                              : p_data.size();
}

gmeans::projection gmeans::calculate_projection(const dataset &p_data,
