...
void kmeans_allocator::allocate(const std::size_t p_amount,
...
void kmedians_allocator::allocate(const std::size_t p_amount,
                                  const dataset& p_data,
                                  cluster_sequence& p_clusters) {
...
void kmedoids_allocator::allocate(const std::size_t p_amount,
                                  const dataset& p_data,
                                  cluster_sequence& p_clusters) {
...
silhouette_ksearch::silhouette_ksearch(
    const std::size_t p_kmin, const std::size_t p_kmax,
    const silhouette_ksearch_allocator::ptr& p_allocator)
    : m_kmin(p_kmin), m_kmax(p_kmax), m_allocator(p_allocator) {
  if (m_kmin <= 1) {
    throw std::invalid_argument(
        "K min value '" + std::to_string(m_kmin) +
        "' should be greater than 1 (impossible to provide silhouette score "
        "for only one cluster).");
  }
}

void silhouette_ksearch::process(const dataset& p_data,
                                 silhouette_ksearch_data& p_result) {
  if (m_kmax > p_data.size()) {
    throw std::invalid_argument("K max value '" + std::to_string(m_kmax) +
                                "' should be bigger than amount of objects '" +
