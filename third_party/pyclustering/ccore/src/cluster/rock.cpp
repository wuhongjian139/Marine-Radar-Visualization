...
rock::rock(const double radius, const std::size_t number_clusters,
           const double threshold)
    : m_adjacency_matrix(adjacency_matrix()),
      m_radius(radius * radius),
      m_degree_normalization(1.0 +
                             2.0 * ((1.0 - threshold) / (1.0 + threshold))),
...
void rock::process(const dataset& p_data, cluster_data& p_result) {
  ...
  /* copy results to the output result (it much more optimal to store in list
   * representation for ROCK algorithm) */
  p_result = rock_data();
  p_result.clusters().insert(p_result.clusters().begin(), m_clusters.begin(),
                             m_clusters.end());
...
bool rock::merge_cluster() {
  ...
  if (cluster1 == cluster2) {
    return false; /* clusters are totally separated (no links between them),
                     it's impossible to made a desicion which of them should be
                     merged */
  }
...
double rock::calculate_goodness(const cluster& cluster1,
  ...
  return number_links /
         (std::pow(size_cluster1 + size_cluster2, m_degree_normalization) -
