...
double ordering_analyser::calculate_connvectivity_radius(
    const ordering& p_ordering, const std::size_t p_amount_clusters,
    const std::size_t p_maximum_iterations) {
  const double maximum_distance =
      *std::max_element(p_ordering.cbegin(), p_ordering.cend());
  ...
  if (extract_cluster_amount(p_ordering, maximum_distance) >
      p_amount_clusters) {
    return result;
  }
...
std::size_t ordering_analyser::extract_cluster_amount(
    const ordering& p_ordering, const double p_radius) {
