...
kmedians::kmedians(const dataset &p_initial_medians, const double p_tolerance,
                   const std::size_t p_max_iter,
...
void kmedians::process(const dataset &data, cluster_data &output_result) {
  ...
  if (data[0].size() != m_initial_medians[0].size()) {
    throw std::invalid_argument(
        "kmedians: dimension of the input data and dimension of the initial "
        "medians must be equal.");
  }
  ...
  for (std::size_t iteration = 0;
       (iteration < m_max_iter) && (changes > m_tolerance) &&
       (counter_repeaters < 10);
       iteration++) {
...
void kmedians::update_clusters(const dataset &p_medians,
  ...
  parallel_for(std::size_t(0), data.size(),
               [this, &p_medians, &labels](std::size_t index) {
                 assign_point_to_cluster(index, p_medians, labels);
               });

  for (std::size_t index_point = 0; index_point < labels.size();
       index_point++) {
...
void kmedians::assign_point_to_cluster(const std::size_t p_index_point,
                                       const dataset &p_medians,
                                       index_sequence &p_lables) {
  size_t index_cluster_optim = 0;
  double distance_optim = std::numeric_limits<double>::max();

  for (size_t index_cluster = 0; index_cluster < p_medians.size();
       index_cluster++) {
    double distance =
        m_metric((*m_ptr_data)[p_index_point], p_medians[index_cluster]);
...
void kmedians::erase_empty_clusters(cluster_sequence &p_clusters) {
  for (std::size_t index_cluster = p_clusters.size() - 1;
       index_cluster != (std::size_t)-1; index_cluster--) {
...
double kmedians::update_medians(cluster_sequence &clusters, dataset &medians) {
  ...
  parallel_for(
      std::size_t(0), clusters.size(),
      [this, &medians, &prev_medians, &clusters,
       &changes](std::size_t index_cluster) {
        calculate_median(clusters[index_cluster], medians[index_cluster]);
        changes[index_cluster] =
            m_metric(prev_medians[index_cluster], medians[index_cluster]);
      });
...
void kmedians::calculate_median(cluster &current_cluster, point &median) {
  ...
  for (size_t index_dimension = 0; index_dimension < dimension;
       index_dimension++) {
    std::sort(current_cluster.begin(), current_cluster.end(),
              [this](std::size_t index_object1, std::size_t index_object2) {
                return (*m_ptr_data)[index_object1] >
                       (*m_ptr_data)[index_object2];
              });

    std::size_t relative_index_median =
        (std::size_t)(current_cluster.size() - 1) / 2;
    std::size_t index_median = current_cluster[relative_index_median];

    if (current_cluster.size() % 2 == 0) {
      std::size_t index_median_second =
          current_cluster[relative_index_median + 1];
      median[index_dimension] = (data[index_median][index_dimension] +
                                 data[index_median_second][index_dimension]) /
                                2.0;
    } else {
      median[index_dimension] = data[index_median][index_dimension];
    }
