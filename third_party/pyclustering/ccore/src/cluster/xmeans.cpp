...
xmeans::xmeans(const dataset &p_initial_centers, const std::size_t p_kmax,
               const double p_tolerance, const splitting_type p_criterion,
               std::size_t p_repeat)
...
double xmeans::improve_parameters(
    cluster_sequence &improved_clusters, dataset &improved_centers,
    const index_sequence &available_indexes) const {
  kmeans_data result;
  kmeans(improved_centers, m_tolerance)
      .process((*m_ptr_data), available_indexes, result);
...
double xmeans::search_optimal_parameters(
    cluster_sequence &improved_clusters, dataset &improved_centers,
    const index_sequence &available_indexes) const {
  ...
  for (std::size_t attempt = 0; attempt < m_repeat; attempt++) {
    ...
    const std::size_t candidates =
        available_indexes.size() < AMOUNT_CENTER_CANDIDATES
            ? available_indexes.size()
            : AMOUNT_CENTER_CANDIDATES;
    kmeans_plus_plus(2U, candidates)
        .initialize(*m_ptr_data, available_indexes, candidate_centers);

    /* perform cluster analysis and update optimum if results became better */
    cluster_sequence candidate_clusters;
    double candidate_wce = improve_parameters(
        candidate_clusters, candidate_centers, available_indexes);
...
void xmeans::improve_structure() {
  ...
  std::vector<dataset> region_allocated_centers(m_ptr_result->clusters().size(),
                                                dataset());

  parallel_for(std::size_t(0), m_ptr_result->clusters().size(),
               [this, &clusters, &current_centers,
                &region_allocated_centers](const std::size_t p_index) {
                 improve_region_structure(clusters[p_index],
                                          current_centers[p_index],
                                          region_allocated_centers[p_index]);
               });
...
void xmeans::improve_region_structure(const cluster &p_cluster,
                                      const point &p_center,
                                      dataset &p_allocated_centers) const {
  ...
  search_optimal_parameters(parent_child_clusters, parent_child_centers,
                            p_cluster);
  ...
  double child_scores =
      splitting_criterion(parent_child_clusters, parent_child_centers);
...
double xmeans::splitting_criterion(const cluster_sequence &analysed_clusters,
                                   const dataset &analysed_centers) const {
  switch (m_criterion) {
    case splitting_type::BAYESIAN_INFORMATION_CRITERION:
      return bayesian_information_criterion(analysed_clusters,
                                            analysed_centers);

    case splitting_type::MINIMUM_NOISELESS_DESCRIPTION_LENGTH:
      return minimum_noiseless_description_length(analysed_clusters,
                                                  analysed_centers);
...
double xmeans::bayesian_information_criterion(
    const cluster_sequence &analysed_clusters,
    const dataset &analysed_centers) const {
  ...
  for (std::size_t index_cluster = 0; index_cluster < analysed_clusters.size();
       index_cluster++) {
    for (auto &index_object : analysed_clusters[index_cluster]) {
      sigma += euclidean_distance_square((*m_ptr_data)[index_object],
                                         analysed_centers[index_cluster]);
    }
  ...
  if (N != K) {
    ...
    /* splitting criterion */
    for (std::size_t index_cluster = 0; index_cluster < analysed_centers.size();
         index_cluster++) {
      double n = (double)analysed_clusters[index_cluster].size();
      double L = n * std::log(n) - n * std::log(N) -
                 n * std::log(2.0 * utils::math::pi) / 2.0 -
                 n * dimension * std::log(sigma) / 2.0 - (n - K) / 2.0;
...
double xmeans::minimum_noiseless_description_length(
    const cluster_sequence &clusters, const dataset &centers) const {
  double score = std::numeric_limits<double>::max();
  ...
  for (std::size_t index_cluster = 0; index_cluster < clusters.size();
       index_cluster++) {
    ...
    for (auto &index_object : clusters[index_cluster]) {
      /* euclidean_distance_square should be used in line with paper, but in
       * this case results are very poor, therefore square root is used to
       * improved. */
      Wi += euclidean_distance((*m_ptr_data)[index_object],
  ...
  if (N - K > 0) {
    ...
    double Ks = (2.0 * alpha * sigma / std::sqrt(N)) *
                std::sqrt((std::pow(alpha, 2)) * sigma_sqrt / N + W - Kw / 2.0);

    score = sigma_sqrt * std::sqrt(2 * K) * (std::sqrt(2 * K) + betta) / N + W -
            sigma_sqrt + Ks + 2 * std::sqrt(alpha) * sigma_sqrt / N;
  }
