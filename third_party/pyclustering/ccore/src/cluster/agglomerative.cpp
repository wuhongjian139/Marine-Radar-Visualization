...
void agglomerative::merge_by_average_link() {
  ...
  for (size_t index_cluster1 = 0; index_cluster1 < m_ptr_clusters->size();
       index_cluster1++) {
    for (size_t index_cluster2 = index_cluster1 + 1;
         index_cluster2 < m_ptr_clusters->size(); index_cluster2++) {
      ...
      for (auto index_object1 : (*m_ptr_clusters)[index_cluster1]) {
        for (auto index_object2 : (*m_ptr_clusters)[index_cluster2]) {
          candidate_average_distance += euclidean_distance_square(
              data[index_object1], data[index_object2]);
        }
      }

      candidate_average_distance /= ((*m_ptr_clusters)[index_cluster1].size() +
                                     (*m_ptr_clusters)[index_cluster2].size());
  ...
  (*m_ptr_clusters)[index1].insert((*m_ptr_clusters)[index1].end(),
                                   (*m_ptr_clusters)[index2].begin(),
                                   (*m_ptr_clusters)[index2].end());
...
void agglomerative::merge_by_centroid_link() {
  ...
  for (size_t index1 = 0; index1 < m_centers.size(); index1++) {
    for (size_t index2 = index1 + 1; index2 < m_centers.size(); index2++) {
      double distance =
          euclidean_distance_square(m_centers[index1], m_centers[index2]);
  ...
  (*m_ptr_clusters)[index_cluster1].insert(
      (*m_ptr_clusters)[index_cluster1].end(),
      (*m_ptr_clusters)[index_cluster2].begin(),
      (*m_ptr_clusters)[index_cluster2].end());
  calculate_center((*m_ptr_clusters)[index_cluster1],
                   m_centers[index_cluster2]);
...
void agglomerative::merge_by_complete_link() {
  ...
  for (size_t index_cluster1 = 0; index_cluster1 < m_ptr_clusters->size();
    ...
    for (size_t index_cluster2 = index_cluster1 + 1;
      ...
      for (auto index_object1 : (*m_ptr_clusters)[index_cluster1]) {
        for (auto index_object2 : (*m_ptr_clusters)[index_cluster2]) {
          double distance = euclidean_distance_square(data[index_object1],
                                                      data[index_object2]);
...
void agglomerative::calculate_center(const cluster& cluster,
                                     point& center) const {
  const std::vector<point>& data = *m_ptr_data;
