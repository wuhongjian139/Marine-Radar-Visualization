...
void random_center_initializer::initialize(const dataset& p_data,
...
void random_center_initializer::create_center(const dataset& p_data,
                                              dataset& p_centers) const {
  std::random_device random_device;

  std::mt19937 engine(random_device());
  engine.seed(static_cast<unsigned int>(
  ...
  index_storage::iterator available_index =
      m_available_indexes.find(random_index_point);
