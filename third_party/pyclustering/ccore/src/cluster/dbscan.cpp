...
dbscan::dbscan(const double p_radius_connectivity,
               const size_t p_minimum_neighbors)
    : m_data_ptr(nullptr),
...
void dbscan::process(const dataset &p_data, const dbscan_data_t p_type,
...
void dbscan::expand_cluster(const std::size_t p_index,
                            cluster &allocated_cluster) {
        ...
        if (neighbor_neighbor_indexes.size() >= m_neighbors) {
          ...
          for (auto neighbor_index : neighbor_neighbor_indexes) {
            ...
            std::vector<std::size_t>::const_iterator position =
                std::find(index_matrix_neighbors.begin(),
                          index_matrix_neighbors.end(), neighbor_index);
...
void dbscan::get_neighbors(const size_t p_index,
                           std::vector<size_t> &p_neighbors) {
  switch (m_type) {
...
void dbscan::get_neighbors_from_points(const size_t p_index,
                                       std::vector<size_t> &p_neighbors) {
  container::kdtree_searcher searcher((*m_data_ptr)[p_index],
                                      m_kdtree.get_root(), m_initial_radius);
  searcher.find_nearest(
      [&p_index, &p_neighbors](const container::kdnode::ptr &node,
...
void dbscan::get_neighbors_from_distance_matrix(
    const size_t p_index, std::vector<size_t> &p_neighbors) {
  ...
  for (std::size_t index_neighbor = 0; index_neighbor < distances.size();
    ...
    if ((candidate_distance <= m_initial_radius) &&
        (index_neighbor != p_index)) {
