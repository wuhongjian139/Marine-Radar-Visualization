...
const std::size_t optics::INVALID_INDEX =
    std::numeric_limits<std::size_t>::max();

optics::optics(const double p_radius, const std::size_t p_neighbors)
    : optics() {
  m_radius = p_radius;
...
void optics::process(const dataset &p_data, const optics_data_t p_type,
                     cluster_data &p_result) {
  m_data_ptr = &p_data;
  ...
  if ((m_amount_clusters > 0) &&
      (m_amount_clusters != m_result_ptr->clusters().size())) {
    double radius = ordering_analyser::calculate_connvectivity_radius(
        m_result_ptr->cluster_ordering(), m_amount_clusters);
...
void optics::initialize() {
  ...
  if (m_optics_objects->empty()) {
    ...
    for (std::size_t i = 0; i < m_data_ptr->size(); i++) {
      m_optics_objects->emplace_back(i, optics::NONE_DISTANCE,
                                     optics::NONE_DISTANCE);
    }
  } else {
    std::for_each(m_optics_objects->begin(), m_optics_objects->end(),
                  [](auto &p_object) { p_object.clear(); });
  }
...
void optics::expand_cluster_order(optics_descriptor &p_object) {
  ...
  if (neighbors.size() >= m_neighbors) {
    ...
    std::multiset<optics_descriptor *, optics_pointer_descriptor_less>
        order_seed;
...
void optics::update_order_seed(
    const optics_descriptor &p_object, const neighbors_collection &p_neighbors,
    std::multiset<optics_descriptor *, optics_pointer_descriptor_less>
        &order_seed) {
  for (auto &descriptor : p_neighbors) {
    ...
    if (!optics_object.m_processed) {
      double reachable_distance =
          std::max({current_reachability_distance, p_object.m_core_distance});
      ...
      if (optics_object.m_reachability_distance == optics::NONE_DISTANCE) {
        ...
        if (reachable_distance < optics_object.m_reachability_distance) {
          ...
          auto object_iterator =
              std::find_if(order_seed.begin(), order_seed.end(),
                           [&optics_object](optics_descriptor *obj) {
...
void optics::extract_clusters() {
  ...
  for (auto optics_object : m_ordered_database) {
    if ((optics_object->m_reachability_distance == optics::NONE_DISTANCE) ||
        (optics_object->m_reachability_distance > m_radius)) {
      if ((optics_object->m_core_distance != optics::NONE_DISTANCE) &&
          (optics_object->m_core_distance <= m_radius)) {
...
void optics::get_neighbors(const size_t p_index,
                           neighbors_collection &p_neighbors) {
  switch (m_type) {
    ...
    default:
      throw std::invalid_argument("Incorrect input data type is specified '" +
                                  std::to_string((unsigned)m_type) + "'");
  }
}

void optics::get_neighbors_from_points(const std::size_t p_index,
                                       neighbors_collection &p_neighbors) {
  p_neighbors.clear();

  container::kdtree_searcher searcher((*m_data_ptr)[p_index],
                                      m_kdtree.get_root(), m_radius);

  container::kdtree_searcher::rule_store rule =
      [&p_index, &p_neighbors](const container::kdnode::ptr &p_node,
                               const double p_distance) {
        if (p_index != (std::size_t)p_node->get_payload()) {
          p_neighbors.emplace((std::size_t)p_node->get_payload(),
                              std::sqrt(p_distance));
        }
      };
...
void optics::get_neighbors_from_distance_matrix(
    const std::size_t p_index, neighbors_collection &p_neighbors) {
  p_neighbors.clear();

  const auto &distances = m_data_ptr->at(p_index);
  for (std::size_t index_neighbor = 0; index_neighbor < distances.size();
       index_neighbor++) {
...
double optics::get_core_distance(
    const neighbors_collection &p_neighbors) const {
...
void optics::calculate_ordering() {
  if (!m_result_ptr->cluster_ordering().empty()) {
    return;
  }
  ...
  for (auto &cluster : clusters) {
    for (auto index_object : cluster) {
      const optics_descriptor &optics_object =
          m_optics_objects->at(index_object);
      if (optics_object.m_reachability_distance != optics::NONE_DISTANCE) {
