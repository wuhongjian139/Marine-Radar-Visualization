...
coordinate_iterator::coordinate_iterator(const std::size_t p_dimension,
                                         const std::size_t p_edge)
    : m_dimension(p_dimension),
      m_edge(p_edge),
      m_coordinate(p_dimension, std::size_t(0)) {}

const clique_block_location &coordinate_iterator::get_coordinate() const
    noexcept {
  return m_coordinate;
}
...
coordinate_iterator &coordinate_iterator::operator++() {
  for (std::size_t index_dimension = 0; index_dimension < m_dimension;
       ++index_dimension) {
...
clique::clique(const std::size_t p_intervals, const std::size_t p_threshold)
    : m_intervals(p_intervals), m_density_threshold(p_threshold) {}
...
void clique::expand_cluster(clique_block &p_block) {
  ...
  if (points.size() <= m_density_threshold) {
    if (!points.empty()) {
      m_result_ptr->noise().insert(m_result_ptr->noise().end(), points.begin(),
                                   points.end());
    }
  ...
  cur_cluster.insert(cur_cluster.end(), p_block.get_points().begin(),
                     p_block.get_points().end());
...
void clique::get_neighbors(const clique_block &p_block,
                           std::list<clique_block *> &p_neighbors) const {
...
void clique::create_grid() {
  ...
  const auto amount_blocks =
      static_cast<std::size_t>(std::pow(m_intervals, dimension));
  ...
  for (std::size_t i = 0; i < amount_blocks; ++i) {
    ...
    m_cells_map.insert({location_to_key(blocks.back().get_logical_location()),
                        &(blocks.back())});
  }
}
...
void clique::get_spatial_location(const clique_block_location &p_location,
                                  const clique::data_info &p_info,
                                  clique_spatial_block &p_block) const {
  ...
  for (std::size_t index_dimension = 0; index_dimension < dimension;
       ++index_dimension) {
    min_corner[index_dimension] +=
        cell_sizes[index_dimension] * p_location[index_dimension];

    if (p_location[index_dimension] == m_intervals - 1) {
      max_corner[index_dimension] = p_info.m_max_corner[index_dimension];
    } else {
      max_corner[index_dimension] =
          min_corner[index_dimension] + cell_sizes[index_dimension];
    }
  }
...
void clique::get_data_info(clique::data_info &p_info) const {
  ...
  for (std::size_t index_dimension = 0; index_dimension < dimension;
       ++index_dimension) {
    p_info.m_sizes.push_back(p_info.m_max_corner[index_dimension] -
                             p_info.m_min_corner[index_dimension]);
  }
}
