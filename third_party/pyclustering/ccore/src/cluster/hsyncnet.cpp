...
void hsyncnet::process(const double order, const solve_type solver,
  ...
  if (current_number_clusters <= m_number_clusters) {
    return; /* Nothing to process, amount of objects is less than required
               amount of clusters. */
  }

  double radius =
      average_neighbor_distance(oscillator_locations, number_neighbors);

  std::size_t increase_step = static_cast<std::size_t>(
      round(oscillator_locations->size() *
            static_cast<std::size_t>(m_increase_persent)));
  if (increase_step < 1) {
...
void hsyncnet::store_state(sync_network_state& state,
                           hsyncnet_analyser& analyser) {
  state.m_time = m_time;
...
double hsyncnet::calculate_radius(const double radius,
                                  const std::size_t amount_neighbors) const {
  double next_radius = 0.0;
  if (amount_neighbors >= oscillator_locations->size()) {
    next_radius = radius * m_increase_persent + radius;
  } else {
    next_radius =
        average_neighbor_distance(oscillator_locations, amount_neighbors);
  }
