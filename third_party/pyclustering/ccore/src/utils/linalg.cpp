...
static sequence action_for_each_component(
...
static sequence action_for_each_component(
    const sequence& a, const double b,
    const std::function<double(double, double)>&& func) {
  sequence result(a.size(), 0.0);
  for (std::size_t i = 0; i < result.size(); i++) {
    result[i] = func(a[i], b);
...
sequence subtract(const sequence& a, const sequence& b) {
  return action_for_each_component(
      a, b, [](double v1, double v2) { return v1 - v2; });
}
...
matrix multiply(const matrix& a, const sequence& b) {
  ...
  if (a.begin()->size() != b.size()) {
    ...
    stream << "Matrix vector (" << a.begin()->size() << ") and vector ("
           << b.size() << ") should have the same size.";
  ...
  for (const auto& v : a) {
    result.push_back(action_for_each_component(
        v, b, [](double v1, double v2) { return v1 * v2; }));
  }
  return result;
}
