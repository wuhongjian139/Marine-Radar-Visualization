...
std::size_t pcnn_network_state::size() const { return m_output.size(); }

pcnn::pcnn() : m_oscillators(0), m_connection(), m_params() {}

pcnn::pcnn(const size_t p_size, const connection_t p_structure,
           const pcnn_parameters &p_parameters) {
  initilize(p_size, p_structure, 0, 0, p_parameters);
}

pcnn::pcnn(const size_t p_size, const connection_t p_structure,
           const size_t p_height, const size_t p_width,
           const pcnn_parameters &p_parameters) {
  initilize(p_size, p_structure, p_height, p_width, p_parameters);
}

void pcnn::simulate(const std::size_t steps, const pcnn_stimulus &stimulus,
...
void pcnn::calculate_states(const pcnn_stimulus &stimulus) {
  ...
  if (stimulus.size() != size()) {
    throw std::out_of_range(
        "pcnn::calculate_states: length of stimulus should be equal to amount "
        "of oscillators in the network.");
  }
  ...
  for (std::size_t index = 0; index < size(); index++) {
    ...
    feeding[index] = m_params.AF * current_oscillator.feeding +
                     stimulus[index] + feeding_influence;
    linking[index] =
        m_params.AL * current_oscillator.linking + linking_influence;

    /* calculate internal activity */
    double internal_activity =
        feeding[index] * (1.0 + m_params.B * linking[index]);
  ...
  for (std::size_t index = 0; index < size(); index++) {
    ...
    oscillator.threshold =
        m_params.AT * oscillator.threshold + m_params.VT * outputs[index];
  }
}

void pcnn::fast_linking(const std::vector<double> &feeding,
                        std::vector<double> &linking,
...
void pcnn::initilize(const size_t p_size, const connection_t p_structure,
                     const size_t p_height, const size_t p_width,
                     const pcnn_parameters &p_parameters) {
  ...
  if (p_size > MAXIMUM_MATRIX_REPRESENTATION_SIZE) {
    m_connection =
        std::shared_ptr<adjacency_collection>(new adjacency_bit_matrix(p_size));
  } else {
    m_connection =
        std::shared_ptr<adjacency_matrix>(new adjacency_matrix(p_size));
  }

  adjacency_connector<adjacency_collection> connector;

  if ((p_height != 0) && (p_width != 0)) {
    connector.create_grid_structure(p_structure, p_width, p_height,
                                    *m_connection);
  } else {
...
void pcnn_dynamic::allocate_sync_ensembles(
    ensemble_data<pcnn_ensemble> &sync_ensembles) const {
  std::unordered_set<std::size_t> traverse_oscillators;
  traverse_oscillators.reserve(oscillators());

  for (const_reverse_iterator iter_state = crbegin(); iter_state != crend();
       ++iter_state) {
...
void pcnn_dynamic::allocate_spike_ensembles(
    ensemble_data<pcnn_ensemble> &spike_ensembles) const {
  for (const_iterator iter_state = cbegin(); iter_state != cend();
       ++iter_state) {
    pcnn_ensemble ensemble;
    const pcnn_network_state &state_network = (*iter_state);
