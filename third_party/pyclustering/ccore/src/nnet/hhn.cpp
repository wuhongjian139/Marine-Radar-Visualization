...
std::size_t hhn_dynamic::size_dynamic() const { return m_size_dynamic; }

std::size_t hhn_dynamic::size_network() const { return m_size_network; }
...
void hhn_dynamic::get_disabled(
    std::set<hhn_dynamic::collect> &p_disabled) const {
  get_collected_types(false, p_disabled);
}

void hhn_dynamic::store(const double p_time,
                        const std::vector<hhn_oscillator> &p_peripheral,
                        const std::vector<central_element> &p_central) {
  ...
  if (m_size_network != 0) {
    if (m_size_network != p_peripheral.size()) {
      throw std::invalid_argument(
          "Amount of neurons on each iteration should be the same.");
    }
  } else {
...
hhn_dynamic::evolution_dynamic &hhn_dynamic::get_peripheral_dynamic(
    const hhn_dynamic::collect &p_type) {
...
hhn_dynamic::evolution_dynamic &hhn_dynamic::get_central_dynamic(
    const hhn_dynamic::collect &p_type) {
...
hhn_dynamic::value_dynamic_ptr hhn_dynamic::get_time() const { return m_time; }

double hhn_dynamic::get_peripheral_value(
    const std::size_t p_iteration, const std::size_t p_index,
    const hhn_dynamic::collect p_type) const {
  return (*m_peripheral_dynamic)[p_type][p_iteration][p_index];
}

double hhn_dynamic::get_central_value(const std::size_t p_iteration,
...
void hhn_dynamic::get_collected_types(
    const bool p_enabled, std::set<hhn_dynamic::collect> &p_types) const {
...
void hhn_dynamic::reserve_collection(const hhn_dynamic::collect p_state,
...
void hhn_dynamic::reserve_dynamic_collection(const hhn_dynamic::collect p_state,
                                             const std::size_t p_size,
                                             network_dynamic &p_dynamic) {
...
void hhn_dynamic::store_membrane_potential(
    const std::vector<hhn_oscillator> &p_peripheral,
    const std::vector<central_element> &p_central) {
  std::vector<double> peripheral_membrane_values(p_peripheral.size(), 0.0);
  for (std::size_t index = 0; index < p_peripheral.size(); index++) {
    peripheral_membrane_values[index] =
        p_peripheral[index].m_membrane_potential;
  }

  m_peripheral_dynamic->at(collect::MEMBRANE_POTENTIAL)
      .emplace_back(std::move(peripheral_membrane_values));
  ...
  m_central_dynamic->at(collect::MEMBRANE_POTENTIAL)
      .emplace_back(std::move(central_membrane_values));
}

void hhn_dynamic::store_active_cond_sodium(
    const std::vector<hhn_oscillator> &p_peripheral,
    const std::vector<central_element> &p_central) {
  ...
  m_peripheral_dynamic->at(collect::ACTIVE_COND_SODIUM)
      .emplace_back(std::move(peripheral_active_sodium));
  ...
  m_central_dynamic->at(collect::ACTIVE_COND_SODIUM)
      .emplace_back(std::move(central_active_sodium));
}

void hhn_dynamic::store_inactive_cond_sodium(
    const std::vector<hhn_oscillator> &p_peripheral,
    const std::vector<central_element> &p_central) {
  std::vector<double> peripheral_inactive_sodium(p_peripheral.size(), 0.0);
  for (std::size_t index = 0; index < p_peripheral.size(); index++) {
    peripheral_inactive_sodium[index] =
        p_peripheral[index].m_inactive_cond_sodium;
  }

  m_peripheral_dynamic->at(collect::INACTIVE_COND_SODIUM)
      .emplace_back(std::move(peripheral_inactive_sodium));
  ...
  m_central_dynamic->at(collect::INACTIVE_COND_SODIUM)
      .emplace_back(std::move(central_inactive_sodium));
}

void hhn_dynamic::store_active_cond_potassium(
    const std::vector<hhn_oscillator> &p_peripheral,
    const std::vector<central_element> &p_central) {
  std::vector<double> peripheral_active_potassium(p_peripheral.size(), 0.0);
  for (std::size_t index = 0; index < p_peripheral.size(); index++) {
    peripheral_active_potassium[index] =
        p_peripheral[index].m_active_cond_potassium;
  }

  m_peripheral_dynamic->at(collect::ACTIVE_COND_POTASSIUM)
      .emplace_back(std::move(peripheral_active_potassium));
  ...
  m_central_dynamic->at(collect::ACTIVE_COND_POTASSIUM)
      .emplace_back(std::move(central_active_potassium));
}
...
std::ostream &operator<<(std::ostream &p_stream, const hhn_dynamic &p_dynamic) {
  const hhn_dynamic::network_dynamic_ptr peripheral =
      p_dynamic.get_peripheral_dynamic();
  const hhn_dynamic::network_dynamic_ptr central =
      p_dynamic.get_central_dynamic();
  ...
  p_stream << p_dynamic.size_dynamic() << " " << p_dynamic.size_network()
           << "\n";
  for (std::size_t index_order = 0; index_order < order_types.size();
       index_order++) {
  ...
  for (std::size_t iter = 0; iter < p_dynamic.size_dynamic(); iter++) {
    ...
    for (std::size_t neuron_index = 0;
         neuron_index < p_dynamic.size_network() + 2; neuron_index++) {
      p_stream << " [ ";
...
hhn_dynamic_reader::hhn_dynamic_reader(const std::string &p_filename)
    : m_filename(p_filename), m_dynamic(nullptr), m_file_stream() {}
...
void hhn_dynamic_reader::extract_size_header(const std::string &p_line,
                                             std::size_t &p_size_dynamic,
                                             std::size_t &p_size_network) {
  std::istringstream string_stream(p_line);
  std::string item;

  if (!std::getline(string_stream, item, ' ')) {
    throw std::invalid_argument(
        "Impossible parse size dynamic from line header: " + p_line);
  }
  ...
  if (!std::getline(string_stream, item, '\n')) {
    throw std::invalid_argument(
        "Impossible parse size network from line header: " + p_line);
  }
...
void hhn_dynamic_reader::extract_enable_header(
    const std::string &p_line, std::vector<hhn_dynamic::collect> &p_collect) {
  std::istringstream string_stream(p_line);
  std::string item;
...
void hhn_dynamic_reader::extract_dynamic(
    const std::string &p_line, double &p_time,
    std::vector<hhn_oscillator> &p_peripheral,
    std::vector<central_element> &p_central) {
  std::istringstream string_stream(p_line);
  std::string item;
  ...
  if (counter_filling != 0) {
    throw std::invalid_argument(
        "Incorrect format of HHN output dynamic: not all neuron states are "
        "found.");
  }
}

void hhn_dynamic_reader::extract_state(std::istringstream &p_stream,
                                       basic_neuron_state &p_state) const {
  std::string item;
  for (std::size_t item_index = 0; std::getline(p_stream, item, ' ');
       item_index++) {
    if (item == "]") {
      return;
    }
    ...
    switch (type_value) {
      ...
      case hhn_dynamic::collect::ACTIVE_COND_POTASSIUM:
        ...
        break;
      default:
        throw std::invalid_argument("Invalid type of value is detected '" +
                                    std::to_string((std::size_t)type_value) +
                                    "'");
    }
  }
}

hhn_network::hhn_network(const std::size_t p_size,
...
void hhn_network::simulate(const std::size_t p_steps, const double p_time,
...
std::size_t hhn_network::size() const { return m_peripheral.size(); }

void hhn_network::store_dynamic(const double p_time, hhn_dynamic &p_dynamic) {
  p_dynamic.store(p_time, m_peripheral, m_central);
}

void hhn_network::calculate_states(const solve_type p_solver,
                                   const double p_time, const double p_step,
                                   const double p_int_step) {
  hhn_states next_peripheral_states(m_peripheral.size());
  calculate_peripheral_states(p_solver, p_time, p_step, p_int_step,
                              next_peripheral_states);

  hhn_states next_central_states(m_central.size());
  calculate_central_states(p_solver, p_time, p_step, p_int_step,
                           next_central_states);

  assign_neuron_states(p_time, p_step, next_peripheral_states,
                       next_central_states);
}

void hhn_network::assign_neuron_states(const double p_time, const double p_step,
                                       const hhn_states &p_next_peripheral,
                                       const hhn_states &p_next_central) {
  for (std::size_t index = 0; index < m_peripheral.size(); index++) {
    ...
    } else if (!((oscillator.m_link_activation_time < p_time) &&
                 (p_time <
                  oscillator.m_link_activation_time + m_params.m_deltah))) {
...
void hhn_network::calculate_peripheral_states(const solve_type p_solver,
                                              const double p_time,
                                              const double p_step,
                                              const double p_int_step,
                                              hhn_states &p_next_states) {
  parallel::parallel_for(std::size_t(0), m_peripheral.size(),
                         [this, &p_solver, p_time, p_step, p_int_step,
                          &p_next_states](const std::size_t p_index) {
                           ...
                           perform_calculation(p_solver, p_time, p_step,
                                               p_int_step, inputs, argv,
                                               p_next_states[p_index]);
                         });
}

void hhn_network::calculate_central_states(const solve_type p_solver,
                                           const double p_time,
                                           const double p_step,
                                           const double p_int_step,
                                           hhn_states &p_next_states) {
  parallel::parallel_for(
      std::size_t(0), m_central.size(),
      [this, &p_solver, p_time, p_step, p_int_step,
       &p_next_states](const std::size_t p_index) {
        std::vector<void *> argv(1, nullptr);
        ...
        perform_calculation(p_solver, p_time, p_step, p_int_step, inputs, argv,
                            p_next_states[p_index]);
      });
}

void hhn_network::perform_calculation(const solve_type p_solver,
                                      const double p_time, const double p_step,
                                      const double p_int_step,
                                      const differ_state<double> &p_inputs,
                                      const differ_extra<> &p_extra,
                                      hhn_state &p_next_states) {
  equation<double> peripheral_equation =
      std::bind(&hhn_network::neuron_states, this, _1, _2, _3, _4);

  switch (p_solver) {
    case solve_type::FORWARD_EULER: {
      throw std::invalid_argument(
          "Forward Euler first-order method is not supported due to low "
          "accuracy.");
    }

    case solve_type::RUNGE_KUTTA_4: {
      std::size_t number_int_steps = (std::size_t)(p_step / p_int_step);
      runge_kutta_4(peripheral_equation, p_inputs, p_time, p_time + p_step,
                    number_int_steps, false, p_extra, p_next_states);
      break;
    }

    case solve_type::RUNGE_KUTTA_FEHLBERG_45: {
      runge_kutta_fehlberg_45(peripheral_equation, p_inputs, p_time,
                              p_time + p_step, 0.00001, false, p_extra,
                              p_next_states);
      break;
    }
    ...
    default: {
      throw std::invalid_argument(
          "Specified differential solver is not supported.");
    }
  }
}

void hhn_network::neuron_states(const double t,
  ...
  double m = inputs[POSITION_ACTIVE_COND_SODIUM]; /* activation conductance of
                                                     the sodium channel (m) */
  double h =
      inputs[POSITION_INACTIVE_COND_SODIUM]; /* inactivaton conductance of the
                                                sodium channel (h)    */
  double n =
      inputs[POSITION_ACTIVE_COND_POTASSIUM]; /* activation conductance of the
                                                 potassium channel (n)  */

  /* Calculate ion current */
  double active_sodium_part =
      m_params.m_gNa * std::pow(m, 3.0) * h * (v - m_params.m_vNa);
  double inactive_sodium_part =
      m_params.m_gK * std::pow(n, 4.0) * (v - m_params.m_vK);
  double active_potassium_part = m_params.m_gL * (v - m_params.m_vL);

  double Iion =
      active_sodium_part + inactive_sodium_part + active_potassium_part;
  ...
  double am =
      ...
      (std::expm1(
          2.5 -
          0.1 * potential)); /* 'exp(x) - 1' can be replaced by 'expm1(x)' */
...
double hhn_network::peripheral_external_current(
    const std::size_t p_index) const {
  return (*m_stimulus)[p_index] *
         (1.0 + 0.01 * generate_uniform_random(-1.0, 1.0));
}

double hhn_network::peripheral_synaptic_current(const std::size_t p_index,
                                                const double p_time,
                                                const double p_membrane) const {
  double memory_impact1 = 0.0;
  for (auto &pulse_time : m_central[0].m_pulse_generation_time) {
    memory_impact1 +=
        alpha_function(p_time - pulse_time, m_params.m_alfa_inhibitory,
                       m_params.m_betta_inhibitory);
  }
  ...
  return m_params.m_w2 * (p_membrane - m_params.m_Vsyninh) * memory_impact1 +
         m_peripheral[p_index].m_link_weight3 *
             (p_membrane - m_params.m_Vsyninh) * memory_impact2;
}

double hhn_network::central_first_synaptic_current(
    const double p_time, const double p_membrane) const {
  double memory_impact = 0.0;
  for (std::size_t index_oscillator = 0; index_oscillator < size();
       index_oscillator++) {
    for (auto &pulse_time :
         m_peripheral[index_oscillator].m_pulse_generation_time) {
      memory_impact +=
          alpha_function(p_time - pulse_time, m_params.m_alfa_excitatory,
                         m_params.m_betta_excitatory);
    }
  }
...
double hhn_network::alpha_function(const double p_time, const double p_alfa,
                                   const double p_betta) {
