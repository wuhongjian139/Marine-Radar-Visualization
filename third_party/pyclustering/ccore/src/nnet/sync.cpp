...
double sync_ordering::calculate_sync_order(
    const std::vector<double> &p_phases) {
  phase_getter getter = [&p_phases](std::size_t index) {
    return p_phases[index];
  };
...
double sync_ordering::calculate_sync_order(
  ...
  phase_getter getter = [&p_oscillators](std::size_t index) {
    return p_oscillators[index].phase;
  };
  return calculate_sync_order_parameter(p_oscillators, getter);
}

template <class TypeContainer>
double sync_ordering::calculate_sync_order_parameter(
  ...
  average_phase = std::exp(std::abs(std::complex<double>(0, 1) *
                                    (average_phase / p_container.size())));
...
double sync_ordering::calculate_local_sync_order(
  ...
  return calculate_local_sync_order_parameter(p_connections, p_oscillators,
                                              getter);
}

template <class TypeContainer>
double sync_ordering::calculate_local_sync_order_parameter(
    const std::shared_ptr<adjacency_collection> &p_connections,
    const TypeContainer &p_container, const phase_getter &p_getter) {
  double exp_amount = 0.0;
...
sync_network::sync_network(const size_t size, const double weight_factor,
                           const double frequency_factor,
                           const connection_t connection_type,
                           const initial_type initial_phases) {
  initialize(size, weight_factor, frequency_factor, connection_type, 0, 0,
             initial_phases);
}
...
sync_network::sync_network(const size_t size, const double weight_factor,
                           ...
                           const initial_type initial_phases) {
  initialize(size, weight_factor, frequency_factor, connection_type, height,
             width, initial_phases);
}

sync_network::~sync_network() {}

void sync_network::initialize(const std::size_t size,
  ...
  if (size > MAXIMUM_MATRIX_REPRESENTATION_SIZE) {
    m_connections =
        std::shared_ptr<adjacency_collection>(new adjacency_bit_matrix(size));
  } else {
    m_connections =
        std::shared_ptr<adjacency_matrix>(new adjacency_matrix(size));
  }

  adjacency_connector<adjacency_collection> connector;

  if ((height != 0) && (width != 0)) {
    connector.create_grid_structure(connection_type, width, height,
                                    *m_connections);
  } else {
  ...
  m_equation =
      std::bind(&sync_network::phase_kuramoto_equation, this, _1, _2, _3, _4);
  ...
  for (std::size_t index = 0; index < size; index++) {
    ...
    switch (initial_phases) {
      ...
      case initial_type::EQUIPARTITION:
        oscillator_context.phase =
            pi / static_cast<double>(size) * static_cast<double>(index);
        break;
      default:
        throw std::runtime_error("Unknown type of initialization");
    }

    oscillator_context.frequency =
        frequency_distribution(generator) * frequency_factor;
  }
}
...
double sync_network::sync_local_order() const {
  return sync_ordering::calculate_local_sync_order(m_connections,
                                                   m_oscillators);
}
...
void sync_network::phase_kuramoto_equation(
    const double t, const differ_state<double> &inputs,
    const differ_extra<void *> &argv, differ_state<double> &outputs) const {
  outputs.resize(1);
  outputs[0] = phase_kuramoto(t, inputs[0], argv);
}

double sync_network::phase_kuramoto(const double t, const double teta,
  ...
  phase = m_oscillators[index].frequency +
          (phase * weight / static_cast<double>(size()));
  return phase;
}

void sync_network::simulate_static(const std::size_t steps, const double time,
...
void sync_network::simulate_dynamic(const double order, const double step,
                                    const solve_type solver,
                                    const bool collect_dynamic,
                                    sync_dynamic &output_dynamic) {
  output_dynamic.clear();
  ...
  for (double time_counter = step; current_order < order;
       time_counter += step) {
    ...
    if (std::abs(current_order - previous_order) < 0.000001) {
      // std::cout << "Warning: sync_network::simulate_dynamic - simulation is
      // aborted due to low level of convergence rate (order = " <<
...
void sync_network::store_dynamic(const double time, const bool collect_dynamic,
...
void sync_network::calculate_phases(const solve_type solver, const double t,
                                    const double step, const double int_step) {
  std::vector<double> next_phases(size(), 0.0);

  parallel_for(std::size_t(0), size(),
               [this, solver, t, step, int_step,
                &next_phases](const std::size_t p_index) {
                 calculate_phase(solver, t, step, int_step, p_index,
                                 next_phases);
               });
...
void sync_network::calculate_phase(const solve_type solver, const double t,
                                   const double step, const double int_step,
  ...
  switch (solver) {
    case solve_type::FORWARD_EULER: {
      double result = m_oscillators[index].phase +
                      phase_kuramoto(t, m_oscillators[index].phase, argv);
    ...
    case solve_type::RUNGE_KUTTA_4: {
      ...
      runge_kutta_4(m_equation, inputs, t, t + step, number_int_steps, false,
                    argv, outputs);
    ...
    case solve_type::RUNGE_KUTTA_FEHLBERG_45: {
      ...
      runge_kutta_fehlberg_45(m_equation, inputs, t, t + step, 0.00001, false,
                              argv, outputs);
      p_next_phases[index] = phase_normalization(outputs[0].state[0]);

      break;
    }
    default: { throw std::runtime_error("Unknown type of solver"); }
  }
}
...
void sync_dynamic::allocate_sync_ensembles(
    const double tolerance, const size_t iteration,
  ...
  for (std::size_t i = 1; i < oscillators(); i++) {
    ...
    for (auto &cluster : ensembles) {
      for (auto &index : cluster) {
        ...
        double phase_shifted =
            std::abs((*last_state_dynamic).m_phase[i] - 2 * pi);

        if (((phase_first < (phase_second + tolerance)) &&
             (phase_first > (phase_second - tolerance))) ||
            ((phase_shifted < (phase_second + tolerance)) &&
             (phase_shifted > (phase_second - tolerance)))) {
...
void sync_dynamic::allocate_sync_ensembles(
    const double tolerance, ensemble_data<sync_ensemble> &ensembles) const {
  allocate_sync_ensembles(tolerance, size() - 1, ensembles);
}

void sync_dynamic::allocate_correlation_matrix(
    sync_corr_matrix &p_matrix) const {
  allocate_correlation_matrix(size() - 1, p_matrix);
}

void sync_dynamic::allocate_correlation_matrix(
    const size_t p_iteration, sync_corr_matrix &p_matrix) const {
...
void sync_dynamic::calculate_order_parameter(
  ...
  for (std::size_t i = start_iteration; i < stop_iteration; i++) {
    const double order_value =
        sync_ordering::calculate_sync_order(at(i).m_phase);
    sequence_order[i - start_iteration] = order_value;
  }
}

void sync_dynamic::calculate_local_order_parameter(
  ...
  for (std::size_t i = start_iteration; i < stop_iteration; i++) {
    const double order_value =
        sync_ordering::calculate_local_sync_order(connections, at(i).m_phase);
