...
som::som(const size_t num_rows, const size_t num_cols,
...
som::som(const som& p_other) { operator=(p_other); }
...
void som::create_connections(const som_conn_type type) {
  ...
  for (int index = 0; index < (int)m_size; index++) {
    ...
    if ((type == som_conn_type::SOM_GRID_EIGHT) ||
        (type == som_conn_type::SOM_GRID_FOUR)) {
      if (upper_index >= 0) {
    ...
    if ((type == som_conn_type::SOM_GRID_EIGHT) ||
        (type == som_conn_type::SOM_GRID_FOUR) ||
        (type == som_conn_type::SOM_HONEYCOMB)) {
      if ((left_index >= 0) &&
          ((int)std::floor((double)left_index / (double)m_cols) ==
           node_row_index)) {
        neuron_neighbors.push_back(left_index);
      }

      if ((int)std::floor((double)right_index / (double)m_cols) ==
          node_row_index) {
    ...
    if (type == som_conn_type::SOM_GRID_EIGHT) {
      if ((upper_left_index >= 0) &&
          ((int)std::floor((double)upper_left_index / (double)m_cols) ==
           upper_row_index)) {
        neuron_neighbors.push_back(upper_left_index);
      }

      if ((upper_right_index >= 0) &&
          ((int)std::floor((double)upper_right_index / (double)m_cols) ==
           upper_row_index)) {
        neuron_neighbors.push_back(upper_right_index);
      }

      if ((lower_left_index < (int)m_size) &&
          ((int)std::floor((double)lower_left_index / (double)m_cols) ==
           lower_row_index)) {
        neuron_neighbors.push_back(lower_left_index);
      }

      if ((lower_right_index < (int)m_size) &&
          ((int)std::floor((double)lower_right_index / (double)m_cols) ==
           lower_row_index)) {
...
void som::create_initial_weights(const som_init_type type) {
  ...
  std::vector<double> maximum_value_dimension(
      dimension, -std::numeric_limits<double>::max());
  std::vector<double> minimum_value_dimension(
      dimension, std::numeric_limits<double>::max());
  ...
  for (size_t dim = 0; dim < dimension; dim++) {
    width_value_dimension[dim] =
        maximum_value_dimension[dim] - minimum_value_dimension[dim];
    center_value_dimension[dim] =
        (maximum_value_dimension[dim] + minimum_value_dimension[dim]) / 2.0;
  }
      ...
      for (size_t i = 0; i < m_size; i++) {
        ...
        for (size_t dim = 0; dim < dimension; dim++) {
          if (dim == 0) {
            if (m_rows > 1) {
              neuron_weight[dim] =
                  minimum_value_dimension[dim] + step_x * neuron_location[dim];
            } else {
            ...
            if (m_cols > 1) {
              neuron_weight[dim] =
                  minimum_value_dimension[dim] + step_y * neuron_location[dim];
            } else {
              neuron_weight[dim] = center_value_dimension[dim];
            }
          } else {
      ...
      for (size_t i = 0; i < m_size; i++) {
        ...
        for (size_t dim = 0; dim < dimension; dim++) {
          std::uniform_real_distribution<double> position_distribution(
              minimum_value_dimension[dim], maximum_value_dimension[dim]);
...
size_t som::adaptation(const size_t index_winner,
  ...
  if (m_conn_type == som_conn_type::SOM_FUNC_NEIGHBOR) {
    for (size_t neuron_index = 0; neuron_index < m_size; neuron_index++) {
      ...
      if (distance < m_local_radius) {
        ...
        for (size_t dim = 0; dim < dimensions; dim++) {
          neuron_weight[dim] +=
              m_learn_rate * influence *
              (input_pattern[dim] - m_weights[neuron_index][dim]);
        }
    ...
    for (size_t dim = 0; dim < dimensions; dim++) {
      neuron_winner_weight[dim] +=
          m_learn_rate * (input_pattern[dim] - neuron_winner_weight[dim]);
    }
    ...
    for (auto& neighbor_index : winner_neighbors) {
      ...
      if (distance < m_local_radius) {
        ...
        for (size_t dim = 0; dim < dimensions; dim++) {
          neighbor_weight[dim] += m_learn_rate * influence *
                                  (input_pattern[dim] - neighbor_weight[dim]);
        }
...
size_t som::train(const dataset& input_data, const size_t num_epochs,
                  bool autostop) {
  ...
  /* store pointer to data (we are not owners, we don't need them after
   * training) */
  m_data = &input_data;
  ...
  for (; epouch < (m_epouchs + 1); epouch++) {
    ...
    m_local_radius = std::pow((m_params.init_radius *
                               std::exp(-((double)epouch / (double)m_epouchs))),
                              2);
    m_learn_rate = m_params.init_learn_rate *
                   std::exp(-((double)epouch / (double)m_epouchs));
    ...
    if (autostop == true) {
      ...
      for (size_t i = 0; i < m_weights.size(); i++) {
        std::copy(m_weights[i].begin(), m_weights[i].end(),
                  m_previous_weights[i].begin());
      }
    }
  }
...
void som::load(const dataset& p_weights, const som_award_sequence& p_awards,
               const som_gain_sequence& p_capture_objects) {
  if (p_weights.size() != m_size) {
    throw std::invalid_argument(
        "Provided weights (" + std::to_string(p_weights.size()) +
        ") for loading does not correspond to network size (" +
  ...
  if (!p_capture_objects.empty()) {
    if (p_capture_objects.size() != m_size) {
      throw std::invalid_argument(
          "Provided capture objects (size '" +
          std::to_string(p_capture_objects.size()) +
          "') for loading does not correspond to network size '" +
          std::to_string(m_size) + "'.");
    }
  ...
  if (!p_awards.empty()) {
    if (p_awards.size() != m_size) {
      throw std::invalid_argument(
          "Provided amount of capture objects by each neuron (size '" +
          std::to_string(p_awards.size()) +
          "') for loading does not correspond to network size '" +
          std::to_string(m_size) + "'.");
    }
...
double som::calculate_maximal_adaptation() const {
  ...
  for (size_t neuron_index = 0; neuron_index < m_size; neuron_index++) {
    ...
    const std::vector<double>& previous_neuron_weight =
        m_previous_weights[neuron_index];

    for (size_t dim = 0; dim < dimensions; dim++) {
      double current_adaptation =
          previous_neuron_weight[dim] - neuron_weight[dim];
