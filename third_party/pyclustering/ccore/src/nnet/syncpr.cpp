...
syncpr::syncpr(const unsigned int num_osc, const double increase_strength1,
               const double increase_strength2)
    :

      sync_network(num_osc, 1, 0, connection_t::CONNECTION_ALL_TO_ALL,
  ...
  equation<double> oscillator_equation =
      std::bind(&syncpr::phase_kuramoto_equation, this, _1, _2, _3, _4);
  set_equation(oscillator_equation);
}

syncpr::syncpr(const unsigned int num_osc, const size_t height,
               const size_t width, const double increase_strength1,
               const double increase_strength2)
    :

      sync_network(num_osc, 1, 0, connection_t::CONNECTION_ALL_TO_ALL, height,
                   width, initial_type::RANDOM_GAUSSIAN),
      m_increase_strength1(increase_strength1),
      m_increase_strength2(increase_strength2),
      m_coupling(num_osc, std::vector<double>(num_osc, 0.0)) {
  equation<double> oscillator_equation =
...
void syncpr::simulate_static(const unsigned int steps, const double time,
...
void syncpr::simulate_dynamic(const syncpr_pattern &input_pattern,
                              const double order, const double step,
                              const solve_type solver,
                              const bool collect_dynamic,
                              syncpr_dynamic &output_dynamic) {
  validate_pattern(input_pattern);
...
double syncpr::calculate_memory_order(
    const syncpr_pattern &input_pattern) const {
  std::complex<double> order(0, 0);

  for (size_t i = 0; i < size(); i++) {
    order += std::complex<double>(input_pattern[i], 0) *
             std::exp(std::complex<double>(0, 1) * m_oscillators[i].phase);
  }
...
double syncpr::phase_kuramoto(const double t, const double teta,
  ...
  for (size_t neighbor_index = 0; neighbor_index < size(); neighbor_index++) {
    if (oscillator_index != neighbor_index) {
      double phase_delta = m_oscillators[neighbor_index].phase -
                           m_oscillators[oscillator_index].phase;

      phase +=
          m_coupling[oscillator_index][neighbor_index] * std::sin(phase_delta);
...
void syncpr::phase_kuramoto_equation(const double t,
...
void syncpr::validate_pattern(const syncpr_pattern &sample) const {
  if (sample.size() != size()) {
    throw syncpr_invalid_pattern(
        "invalid size of the pattern, it should be the same as network size");
  }

  for (size_t i = 0; i < sample.size(); i++) {
    if ((sample[i] != 1) && (sample[i] != -1)) {
      throw syncpr_invalid_pattern(
          "invalid value in the pattern, pattern value should be +1 or -1");
    }
  }
}
