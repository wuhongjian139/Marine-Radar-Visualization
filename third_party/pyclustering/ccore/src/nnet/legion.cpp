...
std::size_t legion_network_state::size() const { return m_output.size(); }

legion_network::legion_network(const size_t num_osc,
...
legion_network::~legion_network() { m_stimulus = nullptr; }

void legion_network::simulate(const unsigned int steps, const double time,
  ...
  for (double cur_time = step; cur_time < time; cur_time += step) {
    ...
    store_dynamic(cur_time, collect_dynamic,
                  output_dynamic); /* store initial state */
  }
}

void legion_network::create_dynamic_connections(
    const legion_stimulus &stimulus) {
  for (size_t i = 0; i < size(); i++) {
    /* re-initialization by 0.0 */
    std::fill(m_dynamic_connections[i].begin(), m_dynamic_connections[i].end(),
              0.0);
    ...
    if (neighbors.size() > 0 && stimulus[i] > 0) {
      ...
      if (number_stimulated_neighbors > 0) {
        double dynamic_weight =
            m_params.Wt / (double)number_stimulated_neighbors;
...
void legion_network::store_dynamic(const double time,
                                   const bool collect_dynamic,
                                   legion_dynamic &dynamic) const {
...
void legion_network::calculate_states(const legion_stimulus &stimulus,
                                      const solve_type solver, const double t,
                                      const double step,
  ...
  for (std::size_t index = 0; index < size(); index++) {
    ...
    differ_state<double> inputs{m_oscillators[index].m_excitatory,
                                m_oscillators[index].m_inhibitory};
    ...
    switch (solver) {
      ...
      case solve_type::RUNGE_KUTTA_4: {
        if (m_params.ENABLE_POTENTIAL) {
          equation<double> neuron_equation =
              std::bind(&legion_network::neuron_states, this, _1, _2, _3, _4);
          runge_kutta_4(neuron_equation, inputs, t, t + step, number_int_steps,
                        false /* only last states */, argv, next_states[index]);
        } else {
          equation<double> neuron_simplify_equation = std::bind(
              &legion_network::neuron_simplify_states, this, _1, _2, _3, _4);
          runge_kutta_4(neuron_simplify_equation, inputs, t, t + step,
                        number_int_steps, false /* only last states */, argv,
                        next_states[index]);
        }
      ...
      case solve_type::RUNGE_KUTTA_FEHLBERG_45: {
        if (m_params.ENABLE_POTENTIAL) {
          equation<double> neuron_equation =
              std::bind(&legion_network::neuron_states, this, _1, _2, _3, _4);
          runge_kutta_fehlberg_45(neuron_equation, inputs, t, t + step, 0.00001,
                                  false /* only last states */, argv,
                                  next_states[index]);
        } else {
          equation<double> neuron_simplify_equation = std::bind(
              &legion_network::neuron_simplify_states, this, _1, _2, _3, _4);
          runge_kutta_fehlberg_45(neuron_simplify_equation, inputs, t, t + step,
                                  0.00001, false /* only last states */, argv,
                                  next_states[index]);
        }
      ...
      default: { throw std::invalid_argument("Not supported solver is used."); }
    }
    ...
    for (auto &index_neighbor : neighbors) {
      coupling += m_dynamic_connections[index][index_neighbor] *
                  heaviside(m_oscillators[index_neighbor].m_excitatory -
                            m_params.teta_x);
    }

    m_oscillators[index].m_buffer_coupling_term =
        coupling -
        m_params.Wz * heaviside(m_global_inhibitor - m_params.teta_xz);
  }

  differ_result<double> inhibitor_next_state;
  differ_state<double> inhibitor_input{m_global_inhibitor};

  equation<double> inhibitor_equation =
      std::bind(&legion_network::inhibitor_state, this, _1, _2, _3, _4);

  switch (solver) {
    case solve_type::RUNGE_KUTTA_4: {
      runge_kutta_4(inhibitor_equation, inhibitor_input, t, t + step,
                    number_int_steps, false /* only last states */, argv,
                    inhibitor_next_state);
      break;
    }
    case solve_type::RUNGE_KUTTA_FEHLBERG_45: {
      runge_kutta_fehlberg_45(inhibitor_equation, inhibitor_input, t, t + step,
                              0.00001, false /* only last states */, argv,
                              inhibitor_next_state);
      break;
    }
...
void legion_network::neuron_states(const double t,
  ...
  double potential_influence =
      heaviside(p + std::exp(-m_params.alpha * t) - m_params.teta);
  ...
  double dx =
      3.0 * x - std::pow(x, 3) + 2.0 - y + stumulus * potential_influence +
      m_oscillators[index].m_coupling_term + m_oscillators[index].m_noise;
  double dy = m_params.eps *
              (m_params.gamma * (1.0 + std::tanh(x / m_params.betta)) - y);
  ...
  for (auto index_neighbor : neighbors) {
    potential +=
        m_params.T *
        heaviside(m_oscillators[index_neighbor].m_excitatory - m_params.teta_x);
  }

  double dp =
      m_params.lamda * (1 - p) * heaviside(potential - m_params.teta_p) -
      m_params.mu * p;
...
void legion_network::neuron_simplify_states(const double t,
                                            const differ_state<double> &inputs,
                                            const differ_extra<void *> &argv,
                                            differ_state<double> &outputs) {
  ...
  double dx = 3.0 * x - std::pow(x, 3) + 2.0 - y + stumulus +
              m_oscillators[index].m_coupling_term +
              m_oscillators[index].m_noise;
  double dy = m_params.eps *
              (m_params.gamma * (1.0 + std::tanh(x / m_params.betta)) - y);
...
void legion_network::inhibitor_state(const double t,
                                     const differ_state<double> &inputs,
                                     const differ_extra<void *> &argv,
...
void legion_network::initialize(const size_t num_osc,
                                const connection_t connection_type,
                                const size_t height, const size_t width,
                                const legion_parameters &params) {
  m_oscillators = std::vector<legion_oscillator>(num_osc, legion_oscillator());
  m_dynamic_connections = std::vector<std::vector<double> >(
      num_osc, std::vector<double>(num_osc, 0.0)),
  m_stimulus = nullptr;
  ...
  if (num_osc > MAXIMUM_MATRIX_REPRESENTATION_SIZE) {
    m_static_connections = std::shared_ptr<adjacency_collection>(
        new adjacency_bit_matrix(num_osc));
  } else {
    m_static_connections =
        std::shared_ptr<adjacency_matrix>(new adjacency_matrix(num_osc));
  }
