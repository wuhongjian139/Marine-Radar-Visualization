...
std::shared_ptr<adjacency_collection>
adjacency_unweight_factory::create_collection(
    const size_t amount_nodes, const adjacency_unweight_t storing_type,
  ...
  switch (storing_type) {
    ...
    default:
      throw std::runtime_error(
          "Unknown type of adjacency collection without weights is required "
          "from the factory.");
  }
...
std::shared_ptr<adjacency_weight_collection>
adjacency_weight_factory::create_collection(
    const size_t amount_nodes, const adjacency_weight_t storing_type,
    const connection_t structure_type,
    const std::function<double()>& weight_value_generator) {
  ...
  /* establish structures between nodes */
  adjacency_weight_connector<adjacency_weight_collection> connector(
