...
kdnode::ptr kdtree::insert(const std::vector<double>& p_point,
                           void* p_payload) {
  kdnode::ptr inserted_kdnode;

  if (m_root == nullptr) {
    kdnode::ptr node = std::make_shared<kdnode>(p_point, p_payload, nullptr,
                                                nullptr, nullptr, 0);
    ...
    while (true) {
      ...
      if (*cur_node <= p_point) {
        if (cur_node->get_right() == nullptr) {
          ...
          cur_node->set_right(std::make_shared<kdnode>(
              p_point, p_payload, nullptr, nullptr, cur_node, discriminator));
      ...
      /* If new node is less than current then check left leaf */
        ...
        if (cur_node->get_left() == nullptr) {
          ...
          cur_node->set_left(std::make_shared<kdnode>(
              p_point, p_payload, nullptr, nullptr, cur_node, discriminator));
...
kdnode::ptr kdtree::recursive_remove(kdnode::ptr& p_node) {
   ...
   */
  kdnode::ptr minimal_node =
      find_minimal_node(p_node->get_right(), discriminator);
...
kdnode::ptr kdtree::find_minimal_node(const kdnode::ptr& p_cur_node,
                                      std::size_t p_discriminator) {
  ...
  for (size_t i = 0; i < candidates.size(); i++) {
    if (candidates[i]->get_value(p_discriminator) <=
        minimal_node->get_value(p_discriminator)) {
...
kdnode::ptr kdtree::find_node_by_rule(const std::vector<double>& p_point,
                                      const kdnode::ptr& p_cur_node,
...
kdnode::ptr kdtree::get_root() const { return m_root; }

std::size_t kdtree::get_size() const { return m_size; }
...
kdtree_searcher::kdtree_searcher(const std::vector<double>& point,
                                 const kdnode::ptr& node,
                                 const double radius_search) {
  initialize(point, node, radius_search);
}

void kdtree_searcher::initialize(const std::vector<double>& point,
                                 const kdnode::ptr& node,
                                 const double radius_search) {
...
void kdtree_searcher::store_if_reachable(const kdnode::ptr& node) const {
  double candidate_distance =
      euclidean_distance_square(m_search_point, node->get_data());
...
void kdtree_searcher::store_user_nodes_if_reachable(
    const kdnode::ptr& node) const {
  double candidate_distance =
      euclidean_distance_square(m_search_point, node->get_data());
  if (candidate_distance <= m_sqrt_distance) {
...
void kdtree_searcher::find_nearest_nodes(
    std::vector<double>& p_distances,
    std::vector<kdnode::ptr>& p_nearest_nodes) const {
  m_proc = std::bind(&kdtree_searcher::store_if_reachable, this,
                     std::placeholders::_1);
...
void kdtree_searcher::find_nearest(const rule_store& p_store_rule) const {
  m_proc = std::bind(&kdtree_searcher::store_user_nodes_if_reachable, this,
                     std::placeholders::_1);
...
kdnode::ptr kdtree_searcher::find_nearest_node() const {
  ...
  m_proc = std::bind(&kdtree_searcher::store_best_if_reachable, this,
                     std::placeholders::_1);
  recursive_nearest_nodes(m_initial_node);
